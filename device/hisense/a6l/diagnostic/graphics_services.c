...
int main(void) {
    ...
    struct binderfs_device b={0};strcpy(b.name,"a6l-v42");need(!ioctl(fd,BINDER_CTL_ADD,&b),"private binder add");close(fd);
    need(!symlink("binderfs/a6l-v42",ROOT "/dev/binder"),"binder alias");
    dir(ROOT "/logs");
    start_logger();
