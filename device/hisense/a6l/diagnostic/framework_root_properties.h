...
static pid_t start_private_properties(void) {
    ...
    int in=open("/v49-olddev/__properties__/property_info",O_RDONLY|O_CLOEXEC);
    ...
    if(!p){
        ...
        for(;;){
            ...
            if(!read_full(c,&cmd,4)&&cmd==0x00020001&&!read_string(c,name,sizeof(name))&&!read_string(c,value,sizeof(value))){
                if(strcmp(name,"service.sf.present_timestamp")==0||strcmp(name,"servicemanager.ready")==0||strcmp(name,"hwservicemanager.ready")==0||strcmp(name,"hwservicemanager.disabled")==0||strncmp(name,"debug.",6)==0||strncmp(name,"sys.system_server.",18)==0||strncmp(name,"persist.sys.",12)==0||strcmp(name,"sys.boot_completed")==0||strcmp(name,"bpf.progs_loaded")==0||strcmp(name,"apexd.status")==0||strcmp(name,"apex.all.ready")==0||strcmp(name,"apexd.config.runtime.erofs_file_backed_mount")==0){prop_write(name,value);result=0;}
