...
#include <sys/wait.h>
#include <unistd.h>
#define ROOT "/tmp/a6l-v44/root"
static pid_t children[8];

static void cleanup(void) {
    for(int i=7;i>=0;i--)if(children[i]>0){kill(-children[i],SIGKILL);waitpid(children[i],NULL,0);children[i]=0;}
}
...
int main(void) {
    setbuf(stdout,NULL);atexit(cleanup);signal(SIGALRM,timedout);signal(SIGTERM,timedout);alarm(100);
    ...
    struct binderfs_device b={0};strcpy(b.name,"a6l-v44");need(!ioctl(fd,BINDER_CTL_ADD,&b),"private binder add");strcpy(b.name,"a6l-v44-hw");need(!ioctl(fd,BINDER_CTL_ADD,&b),"private hwbinder add");close(fd);
    need(!symlink("binderfs/a6l-v44",ROOT "/dev/binder"),"binder alias");
    need(!symlink("binderfs/a6l-v44-hw",ROOT "/dev/hwbinder"),"hwbinder alias");
    dir(ROOT "/logs");
    start_logger();
    ...
    pid_t c=start(3,"/system/bin/a6l_surface_client",0,"client");int status=0;
    ...
    puts("A6L_SURFACE_SERVICES_PASS client=1 services_alive=5 private_properties=1 namespace_cleanup=1");return 0;
}
