#!/bin/bash
# camfix2 build (25 Sep 2026): camfix patches + camfix2_patch.py (VFE pm-domain refcount = stream-off oops fix,
# sdm660 ICC vote, CSIPHY/CSID/ISPIF/VFE diagnostics) -> firmware/extracted/camera-20260925 + laptop v75/camera3.
set -u
...
B=/home/a6l/camfix2; A=$W/firmware/extracted/camera-20260925; OLD=$W/firmware/extracted/camera-20260924b
P=$W/device/hisense/a6l/kernel/camera
export PATH="$CL:$PATH" KBUILD_BUILD_USER=a6l KBUILD_BUILD_HOST=a6l-build
step(){ echo; echo "=== $(date +%T) $*"; }
rm -rf $B; mkdir -p $B/mod $B/camss $B/orig $B/ovl $B/src-snapshot $A/extra
step sources+patch
...
(cd $B && diff -ru orig/camss camss) > $P/patches/camss-sdm660-camfix2-cumulative.patch
...
ls -la $B/camss/qcom-camss.ko && modinfo $B/camss/qcom-camss.ko | grep -E "vermagic|^parm"; nm $B/camss/qcom-camss.ko | grep -E " [Uu] " | grep -iE "icc|a6l" 
...
step assemble $A
cp $B/camss/*.c $B/camss/*.h $B/src-snapshot/ 2>/dev/null; cp $B/mod/v4l2-cci.ko $B/mod/imx576_a6l.ko $B/mod/s5k3t1.ko $B/mod/gt9769.ko $B/mod/hi846.ko $B/mod/i2c-qcom-cci.ko $B/camss/qcom-camss.ko $B/a6l-camera-v75.dtbo $B/a6l_camcap $A/
...
cd $W/.relay
./lap.sh 30 'mkdir -p ~/A6L-usb-20260915/v75/camera3/extra && echo LAPTOP_MKDIR_OK'
timeout 120 $SCP 'C:/Users/Pierre/Desktop/A6L/firmware/extracted/camera-20260925/*' a6l-laptop:A6L-usb-20260915/v75/camera3/ 2>&1 | tail -3
timeout 60 $SCP 'C:/Users/Pierre/Desktop/A6L/firmware/extracted/camera-20260925/extra/a6l_cam_ovl.ko' a6l-laptop:A6L-usb-20260915/v75/camera3/extra/ 2>&1 | tail -3
./lap.sh 40 'cd ~/A6L-usb-20260915/v75/camera3 && sha256sum -c SHA256SUMS 2>&1 | grep -vc ": OK$"; sha256sum -c SHA256SUMS 2>&1 | grep -c ": OK$"'
echo CAMFIX2_BUILD_DONE
