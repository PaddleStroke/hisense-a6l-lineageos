...
extern int msm_dsi_a6l_reslave(void);
static bool reslave = true;	/* 22 Sep 2026: DSI1 PHY must be re-slaved to PLL0 while video runs, then the bridge ACKs */
module_param(reslave, bool, 0644);
...
struct a6l_epd_dsi {
	...
	bool hv_on;
	bool bridge_ok;
	struct delayed_work bridge_work;
};
...
static int a6l_bridge_init(struct device *dev, const char *when)
{
	...
	if (!ret && bridge == 1) {
		/* like stock / the userspace tool: pace the writes, keep going on NAK, then verify and rewrite */
		int pass, bad = 0, nak = 0;

		for (i = 0; i < ARRAY_SIZE(a6l_tc358767_table); i++) {
			u16 r = a6l_tc358767_table[i].reg;
			u32 back;

			if (a6l_bridge_wr(a, r, a6l_tc358767_table[i].val))
				nak++;
			a6l_bridge_rd(a, r, &back);
			if (r == 0x0904 || r == 0x0908 || r == 0x0918 || r == 0x0800)
		...
		for (pass = 0; pass < 3; pass++) {
			...
			for (i = 0; i < ARRAY_SIZE(a6l_tc358767_table); i++) {
				...
				if (r == 0x0904 || r == 0x0908 || r == 0x0800 || r == 0x0104 || r == 0x0204 || r == 0x0510)
					continue;	/* self-clearing / start bits */
				if (a6l_bridge_rd(a, r, &back) || back != a6l_tc358767_table[i].val) {
					bad++;
					a6l_bridge_wr(a, r, a6l_tc358767_table[i].val);
					usleep_range(1000, 1500);
				}
			}
			if (!bad)
		...
		ret = bad ? -EIO : 0;
		dev_info(dev, "bridge (%s): stock table written, %d NAKs on first pass, verify %s after %d pass(es)\n",
			 when, nak, bad ? "FAILED" : "OK", pass + 1);
	}
	i2c_put_adapter(a);
	return ret;
}
...
static void a6l_bridge_work_fn(struct work_struct *w)
{
	struct a6l_epd_dsi *ctx = container_of(to_delayed_work(w), struct a6l_epd_dsi, bridge_work);
	int r = msm_dsi_a6l_reslave();

	dev_info(ctx->panel.dev, "bridge (work): reslave ret=%d\n", r);
	msleep(300);	/* the manual replay that works leaves ~0.3-1 s before the table */
	ctx->bridge_ok = !a6l_bridge_init(ctx->panel.dev, "work");
}
...
static int a6l_epd_dsi_enable(struct drm_panel *panel)
{
	...
	if (reslave) {
		schedule_delayed_work(&ctx->bridge_work, msecs_to_jiffies(100));
		return 0;
	}
	if (!ctx->bridge_ok)
...
static int a6l_epd_dsi_disable(struct drm_panel *panel)
{
	...
	cancel_delayed_work_sync(&ctx->bridge_work);
	ctx->bridge_ok = false;
	return 0;
}
...
static int a6l_epd_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	INIT_DELAYED_WORK(&ctx->bridge_work, a6l_bridge_work_fn);
