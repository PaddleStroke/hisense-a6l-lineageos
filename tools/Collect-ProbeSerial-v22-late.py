...
last_usb = None
boot_deadline = None
late_capture_started = time.monotonic()
...
try:
    with (args.output / 'console.bin').open('wb') as log:
        while time.monotonic() < deadline and (boot_deadline is None or time.monotonic() < boot_deadline):
            state = {}
            ...
            if (boot_deadline is None and last_usb and last_usb.get("idVendor") == "18d1"
                ...
                boot_deadline = time.monotonic() + 55
                report["recovery_departure_utc"] = datetime.now(timezone.utc).isoformat()
                report["post_selection_capture_seconds"] = 55
            if state != last_usb:
            ...
            if chunk:
                ...
                save()
                if (time.monotonic() - late_capture_started >= 35 and report['ready_seen'] and b'A6L_SERIAL_WRITE_END' in payload
                        and any(int(v) >= 40 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+)', payload))):
                    break
    ...
    report['staged_capture_complete'] = (time.monotonic() - late_capture_started >= 35 and b'A6L_SERIAL_WRITE_END' in payload and any(int(v) >= 40 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+)', payload)))
