...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v8-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v8.py';Capture='Run-LaptopProbeCapture-v8.py';Restore='Run-LaptopDiagnosticRestore-v8.py'}[$Action]
