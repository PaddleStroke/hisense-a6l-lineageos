...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v5-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v5.py';Capture='Run-LaptopProbeCapture-v5.py';Restore='Run-LaptopDiagnosticRestore-v5.py'}[$Action]
