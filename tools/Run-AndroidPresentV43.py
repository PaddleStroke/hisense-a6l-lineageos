...
OUT=ROOT/'capture-present-user-v43/graphics';PACKAGE=ROOT/'android-present-v43';SERIAL='HLTE730T-PROBE'
def main():
    ...
    try:
        ...
        assert shell('/system/bin/toybox','ls','-d','/tmp/a6l-v43',required=False)['exit']!=0
        cards=shell('/system/bin/toybox','ls','/sys/class/drm')['stdout'].split();assert not any(s.startswith('card') for s in cards),cards
        shell('/system/bin/toybox','mkdir','-m','0700','/tmp/a6l-v43')
        run(['adb','-s',SERIAL,'push',str(PACKAGE/'payload')+'/.','/tmp/a6l-v43/'],timeout=60)
        executables=['/tmp/a6l-v43/'+name for name,info in files.items() if info['mode']&0o111]
        shell('/system/bin/toybox','chmod','0755',*executables)
        hashes=shell('sha256sum',*['/tmp/a6l-v43/'+name for name in files],timeout=20)['stdout']
        actual={l.split()[1]:l.split()[0] for l in hashes.splitlines()}
        assert actual=={'/tmp/a6l-v43/'+n:i['sha256'] for n,i in files.items()}
        ...
        shell('/system/bin/toybox','insmod','/tmp/a6l-v43/a6l_simplefb.ko',timeout=15)
        p=shell('/tmp/a6l-v43/bin/a6l_graphics_services',timeout=75,required=False)
        (OUT/'supervisor.log').write_text(p['stdout']+p['stderr'])
        run(['adb','-s',SERIAL,'pull','/tmp/a6l-v43/root/logs',str(OUT/'logs')],timeout=15,required=False)
        ...
        report['presentation_passed']='A6L_PRESENT_PASS frames=4 buffers=2 commands=checked fences=checked' in client
        ...
        report['passed']=report['presentation_passed'] and report['graphics_passed'] and report['global_mounts_unchanged'] and report['readiness_property_restored'] and report['adb_alive']
