...
$resultName = if ($DiagnosticInstall) { 'interactive-stock-control-install-v1-result.json' } elseif ($DiagnosticRestore) { 'interactive-stock-control-restore-v1-result.json' } elseif ($ProbeCapture) { 'interactive-stock-control-capture-v1-result.json' } elseif ($RecoveryPoweroff) { 'interactive-recovery-poweroff-v1-result.json' } elseif ($RecoveryRestore) { 'interactive-recovery-restore-v1-result.json' } elseif ($ButtonEntry) { 'interactive-button-entry-v2-result.json' } elseif ($RecoveryAccess) { 'interactive-recovery-access-v4-result.json' } elseif ($VendorUnlock) { 'interactive-vendor-unlock-result.json' } elseif ($VendorQuery) { 'interactive-vendor-query-result.json' } elseif ($RamTransfer) { 'interactive-ram-transfer-result.json' } elseif ($NoLpm) { 'interactive-no-lpm-test-result.json' } elseif ($GoogleClient) { 'interactive-google-test-result.json' } elseif ($PauseFwupd) { 'interactive-quiet-test-result.json' } else { 'interactive-test-result.json' }
...
if ($DiagnosticInstall) {
    $Host.UI.RawUI.WindowTitle = 'A6L - Stock recovery control installation'
    Write-Host 'Installing the fixed stock recovery control after exact spare, GPT, stock and empty boot-message checks.'
...
try {
    $remoteArgs = if ($DiagnosticInstall) {
        @('python3', '/home/pierrelouis/A6L-usb-20260915/Run-LaptopStockControlInstall-v1.py')
    } elseif ($DiagnosticRestore) {
        @('python3', '/home/pierrelouis/A6L-usb-20260915/Run-LaptopStockControlRestore-v1.py')
    } elseif ($ProbeCapture) {
        @('python3', '/home/pierrelouis/A6L-usb-20260915/Run-LaptopStockControlCapture-v1.py')
    } elseif ($RecoveryPoweroff) {
