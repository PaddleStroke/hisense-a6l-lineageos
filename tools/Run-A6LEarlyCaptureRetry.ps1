param([Parameter(Mandatory=$true)][ValidateSet('Capture')][string]$Action)
$ErrorActionPreference = 'Stop'
$taskRoot = Split-Path -Parent $PSScriptRoot
$accessDir = Join-Path $taskRoot 'logs/laptop-access'
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v4b-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v4.py';Capture='Run-LaptopProbeCapture-v4b.py';Restore='Run-LaptopDiagnosticRestore-v4.py'}[$Action]
