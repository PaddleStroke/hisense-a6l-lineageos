...
CAPTURE = ROOT / 'capture-probe-serial-user-v17'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v17/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '50bd78a2a94a7d151defbe637b28492207bdd6b5f857cc413175b13535658c15'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '340s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v17.py'),
