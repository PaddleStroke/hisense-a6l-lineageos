...
ARCHIVE = ROOT / 'firmware/extracted/staged-usb-probe-v14-20260916'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-staged-usb-probe-v14-20260916')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/staged-usb-ramdisk-v14-20260916/ramdisk.cpio.gz'),
    ...
    try:
        with (p / 'console.log').open('wb') as log, (p / 'stderr.log').open('wb') as err:
            ...
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                ...
                while time.monotonic() < until:
                    ...
                    if (any(int(v) >= 122 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+) ', data))) or b'Kernel panic' in data:
                        break
        ...
        checks = dict(init_milestones_complete=all(pos >= 0 for pos in positions),
                      ...
                      alive=any(int(v) >= 122 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+) ', data)),
                      staged_schedule=b'A6L_STAGED_USB_V14 config_after=60 bind_after=90 serial_after=120 heartbeat=2' in data,
...
def main():
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 122 seconds; no physical USB emulation',
              'kernel_sha256': hashlib.sha256(adapted).hexdigest(),
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/staged-usb-ramdisk-v14-20260916/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
