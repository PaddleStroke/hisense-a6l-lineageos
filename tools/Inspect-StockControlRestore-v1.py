...
spec = importlib.util.spec_from_file_location('writer', root / 'Write-LaptopStockControlRestore-v1.py')
writer = importlib.util.module_from_spec(spec)
spec.loader.exec_module(writer)
import StockControlProtocol as protocol
digests = {}
for mode, name in [('restore-stock', 'stock-recovery/restore-stock-recovery.img'), ('install-control', 'ram-staging/recovery-stock-control-no-footer.img')]:
...
print(json.dumps({'scope': 'Offline restore imports/geometry/payload check; no USB opened',
                  ...
                  'worker_sha256': hashlib.sha256((root / 'Write-LaptopStockControlRestore-v1.py').read_bytes()).hexdigest(),
                  'protocol_sha256': hashlib.sha256((root / 'StockControlProtocol.py').read_bytes()).hexdigest(),
