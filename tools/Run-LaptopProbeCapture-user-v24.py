...
CAPTURE = ROOT / 'capture-probe-serial-user-v24'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v24/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '6cc9c9940c068057ed58f3a210213b444ef2321eee01e1aa96b082e275a28731'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '340s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v24.py'),
