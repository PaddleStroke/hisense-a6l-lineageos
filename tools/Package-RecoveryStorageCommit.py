...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-stages-v29-20260916'
RAM = ROOT / 'firmware/extracted/storage-commit-ramdisk-v30-20260916'
TEST = ROOT / 'firmware/extracted/storage-commit-probe-v30-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-commit-smoke-v30-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == 'f296f2f72194da3eb9fd9854b282366ba06d9148971f134067d81c5cfb1f442f'
    ...
    assert sha(original) == '95b98dd09d5f12703553640ecb0cf6aa109e3ef6ad7bbd98698385d2dfc9d7b6'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V30 isolated commit stage and PM-state trace; first CMD0: baseline, signal mask, one command, bounded status branch, normal IRQ on success; same DT/RAM/module',
