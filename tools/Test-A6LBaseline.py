...
ARCHIVE = ROOT / 'firmware/extracted/baseline-6.19-probe-r2-20260917'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-baseline-6.19-probe-r2-20260917')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/baseline-6.19-ramdisk-20260917/ramdisk.cpio.gz'),
            '-append', 'earlycon=a6lfb keep_bootcon console=ttyAMA0,115200 loglevel=8 panic=0 initcall_debug pd_ignore_unused a6l_manual_usb=1 a6l_usb_trace=1 a6l_storage_trace=1']
    ...
    try:
        ...
        checks = dict(pid1_ready=b'A6L_RAM_PROBE_READY' in data,
...
def main():
    ...
    image = (ROOT / 'firmware/extracted/baseline-6.19-kernel-20260917/Image').read_bytes()
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 22 seconds; no physical USB emulation',
              ...
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/baseline-6.19-ramdisk-20260917/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
        for name, board, size in [('enabled', 'hisense,hlte730t', 0x23ff000), ('wrong-board', 'linux,dummy-virt', 0x23ff000), ('short-reservation', 'hisense,hlte730t', 0x1000)]:
