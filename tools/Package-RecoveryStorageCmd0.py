...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-scan-v24-20260916'
RAM = ROOT / 'firmware/extracted/storage-cmd0-ramdisk-v25-20260916'
TEST = ROOT / 'firmware/extracted/storage-cmd0-probe-v25-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-cmd0-smoke-v25-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '386202084d486afaee0c9bf12ea412e0e8402c5026091498d19b66070330099b'
    ...
    assert sha(original) == '6cc9c9940c068057ed58f3a210213b444ef2321eee01e1aa96b082e275a28731'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V24 DT/command line/init/module/power settings preserved; add16 CMD0 process-context checkpoints with unchanged96-pause bound',
