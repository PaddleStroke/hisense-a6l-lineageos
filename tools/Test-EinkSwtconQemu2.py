"""Run the recovered stock software-TCON call sequence (Init/ModeDecision/Update) in diskless QEMU.
usage: Test-EinkSwtconQemu.py <attempt> [waveform.bin]   No phone access; frames are meaningless without the real SPI waveform."""
import gzip,hashlib,json,re,shutil,subprocess,time
from pathlib import Path
import sys,datetime
N=sys.argv[1];WF=Path(sys.argv[2]) if len(sys.argv)>2 else None;DUMP=[a for a in sys.argv[3:] if a.startswith('--dump')];EXTRA=' '.join(a for a in sys.argv[3:] if a.startswith('--pattern='));TEMP=(DUMP[0].split('=')+['25'])[1] if DUMP else '25';STAMP=datetime.date.today().strftime('%Y%m%d')
...
copy(Path('/home/a6l/android/a6l-lineage24/out/target/product/a6l/system/bin/a6l_eink_swtcon_probe'),ARCH/'a6l_eink_swtcon_probe')
copy(ROOT/'firmware/extracted/vendor/lib64/libtcon_eink.so',LIBS/'libtcon_eink.so')
queue=[ARCH/'a6l_eink_swtcon_probe',LIBS/'libtcon_eink.so'];seen=set()
while queue:
    p=queue.pop()
...
if WF:copy(WF,ARCH/'waveform.bin')
...
'''.replace('WFARG',('/eink/waveform.bin' if WF else '')+(f' --dump --temp={TEMP}' if DUMP else '')+(' '+EXTRA if EXTRA else '')))
