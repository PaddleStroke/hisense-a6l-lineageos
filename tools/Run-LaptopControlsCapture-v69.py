...
CAPTURE = ROOT / 'capture-controls-user-v69'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V69 combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v69', 'suspend', 'idle']):
            raise RuntimeError('V69 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v69/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '4dc4861ebcbd30b5f38ab236b3bdb37e57140803dca1b993d4749808c2815eec'):
