...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-hold-v28-20260916'
RAM = ROOT / 'firmware/extracted/storage-stages-ramdisk-v29-20260916'
TEST = ROOT / 'firmware/extracted/storage-stages-probe-v29-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-stages-smoke-v29-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '39f8a4b4958294650b5ab05d25829efc66e3168912d36e171e5833a5416cf179'
    ...
    assert sha(original) == 'da70c8383db73370f75af2838f2701efcab5901a183d8318b7a67c760f3104b6'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V29 staged first CMD0: baseline, signal mask, one command, bounded status branch, normal IRQ on success; same DT/RAM/module',
