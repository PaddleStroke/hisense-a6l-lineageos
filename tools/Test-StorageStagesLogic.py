"""Run the actual V29 C state machine against bounded fake SDHCI registers."""
from pathlib import Path
import re, subprocess, json
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'firmware/extracted/storage-stages-logic-v29-20260916'
source=ROOT/'tools/a6l-cmd0-stages-v29.c'
...
struct device { int unused; }; struct work_struct { int unused; };
...
static void issue(void) { start(); tick(); assert(signals==1 && commands==0); tick(); assert(commands==1); }
...
int main(void) {
 ...
 puts("PASS: 13 guarded stage and error-path scenarios; one command at most; no status acknowledgement or INT_ENABLE mutation");
}
'''
(OUT/'harness.c').write_text(pre+'\n'+'\n'.join(defs[n] for n in sorted(needed))+'\n'+post+'\n'+source.read_text()+'\n'+test)
binary=Path('/tmp/a6l-v29-stage-test')
...
(OUT/'report.json').write_text(json.dumps({'passed':True,'scenarios':13,'result':r.stdout,'limits':'Actual C branch logic with fake registers; not hardware, IRQ concurrency or electrical validation.'},indent=2))
print(r.stdout)
