...
flock -n 9
# Preserve the compiled V11 DTB. This target has a separate name.
for name in sdm660-hisense-a6l-probe.dts sdm660-hisense-a6l-recovery.dts; do
    cmp "$workspace/device/hisense/a6l/kernel/$name" "$kernel_dir/arch/arm64/boot/dts/qcom/$name"
done
cp "$workspace/device/hisense/a6l/kernel/sdm660-hisense-a6l-usb-only.dts" "$kernel_dir/arch/arm64/boot/dts/qcom/"
cd "$kernel_dir"
make O="$out" ARCH=arm64 LLVM=1 DTC_FLAGS=-@ -j4 qcom/sdm660-hisense-a6l-usb-only.dtb
