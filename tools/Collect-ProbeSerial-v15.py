...
try:
        ...
        while time.monotonic() < deadline:
            ...
            if chunk:
                ...
                if (report['ready_seen'] and b'A6L_SERIAL_WRITE_END' in payload
                        and any(int(v) >= 152 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+)', payload))):
                    break
    ...
    report['staged_capture_complete'] = (b'A6L_SERIAL_WRITE_END' in payload and any(int(v) >= 152 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+)', payload)))
