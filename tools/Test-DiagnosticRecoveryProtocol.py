...
class WriterChecks(unittest.TestCase):
    ...
    def test_cross_mode_payload_and_unrecognized_modes_rejected(self):
        ...
        directory = Path(__file__).resolve().parents[1] / 'firmware/extracted/recovery-probe-20260914'
        if not directory.exists():
            directory = Path(__file__).resolve().parent
            files = [('install-diagnostic', directory / 'ram-staging/recovery-diagnostic-unsigned.img'),
            ...
            files = [('install-diagnostic', directory / 'recovery-diagnostic-unsigned.img'),
                     ('restore-stock', directory / 'restore-stock-recovery.img')]
