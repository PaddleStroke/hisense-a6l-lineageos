...
CAPTURE = ROOT / 'capture-probe-serial-v1'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        install = json.loads((ROOT / 'capture-diagnostic-install-v1/edl/report.json').read_text())
        ...
        save()
        print('READY: diagnostic log capture is armed for three minutes. Follow the physical recovery boot instructions.', flush=True)
        boot_requested = True
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '190s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v1.py'),
                                     str(CAPTURE / 'serial'), '--seconds', '180'],
