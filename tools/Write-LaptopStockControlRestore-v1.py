...
from StockControlProtocol import check_program_xml, program_recovery, verify_payload, TARGETS
...
def main():
    ...
    parser.add_argument('--mode', choices=('restore-stock',), required=True)
    ...
    capture_name = 'capture-stock-control-install-v1' if args.mode == 'install-control' else 'capture-stock-control-restore-v1'
    ...
    try:
        ...
        payload = ((home / 'ram-staging/recovery-stock-control-no-footer.img').read_bytes()
                   if args.mode == 'install-control' else stock)
        ...
        def guarded_xml(self, data, *positional, **keywords):
            try:
                ...
                if tag == 'power':
                    ...
                    expected = 'off' if args.mode == 'install-control' else 'reset'
        ...
        for name, (offset, size) in REGIONS.items():
            ...
            if name == 'recovery' and args.mode == 'install-control' and digest != RECOVERY_HASH:
                raise ValueError('Recovery differs from the verified stock restore image')
            print('Verified fixed read: ' + name, flush=True)
        if args.mode == 'install-control' and any((args.output / 'misc-bcb.bin').read_bytes()):
        ...
        value = 'off' if args.mode == 'install-control' else 'reset'
