...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-noled-v22-20260916'
RAM = ROOT / 'firmware/extracted/storage-host-ramdisk-v23-20260916'
TEST = ROOT / 'firmware/extracted/storage-host-probe-v23-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-host-smoke-v23-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '230bab281c9ece9bdd724b801355fec53ba5653fef59e57a04bdafdf0d6f74cc'
    ...
    assert sha(original) == '19ea35f9ce8b97683c195aa2a26fce811170a04064483c539eddaba1782720d3'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V22 DT/command line/init/no-LED module/power settings preserved; add bounded MMC host and initial power-up checkpoints',
