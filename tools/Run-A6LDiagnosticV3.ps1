...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v3-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v3.py';Capture='Run-LaptopProbeCapture-v3.py';Restore='Run-LaptopDiagnosticRestore-v3.py'}[$Action]
$Host.UI.RawUI.WindowTitle = 'A6L - Corrected recovery - ' + $Action
Write-Host ('A6L corrected recovery: ' + $Action) -ForegroundColor Cyan
