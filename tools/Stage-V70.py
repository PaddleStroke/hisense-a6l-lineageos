"""Copy the V70 preparation to the laptop only when explicitly run; never launches anything on the phone.
...
manifest = json.loads((T / 'diagnostic-user-v70-tools.json').read_text())
assert manifest['candidate_sha256'] == V70
...
assert sha(image / 'recovery-diagnostic-unsigned.img') == V70
assert json.loads((image / 'captured-abl-validation.json').read_text())['passed']
bundle = ROOT / 'firmware/extracted/v70-attended-bundle-20260921'
...
files = {name: T / name for name in manifest['files'] if name != 'diagnostic-user-v70-tools.json'}
files['diagnostic-user-v70-tools.json'] = T / 'diagnostic-user-v70-tools.json'
files['Verify-ControlsV70Stage.py'] = T / 'Verify-ControlsV70Stage.py'
files['v70/captured-abl-validation.json'] = image / 'captured-abl-validation.json'
files['v70/bundle/manifest.json'] = bundle / 'manifest.json'
for name in bundle_files:
    files['v70/bundle/' + name] = bundle / name
files['ram-staging/recovery-diagnostic-staged-usb-v70.img'] = image / 'recovery-diagnostic-unsigned.img'
pins = {name: sha(path) for name, path in files.items()}
(T / 'v70-pins.json').write_text(json.dumps(pins, indent=2) + '\n')
files['v70-pins.json'] = T / 'v70-pins.json'

preflight = "import pathlib; r=pathlib.Path('" + REMOTE + "'); assert r.is_dir(); assert not (r/'capture-diagnostic-install-user-v70').exists(); assert not (r/'capture-diagnostic-restore-user-v70').exists(); print('STAGING_ONLY_READY')"
...
out = ROOT / 'research/recovery-v70-20260921'; out.mkdir(exist_ok=True)
result = run([SSH, '-F', CONFIG, 'a6l-laptop', 'python3 ' + REMOTE + '/Verify-ControlsV70Stage.py'])
(out / 'laptop-stage-verification.json').write_text(result)
inspect = run([SSH, '-F', CONFIG, 'a6l-laptop', 'python3 ' + REMOTE + '/Inspect-DiagnosticRecovery-user-v70.py'])
