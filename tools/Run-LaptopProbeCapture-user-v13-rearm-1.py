...
CAPTURE = ROOT / 'capture-probe-serial-user-v13-rearm-1'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v13/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'f302bc49910f0c1595590b5dc7ac062f86681ff9b26233142e93cb8e501c661e'):
        ...
        if not all(word in inhibition for word in ['A6L-v13-rearm-1', 'suspend', 'idle']):
            raise RuntimeError('Expected desktop idle/suspend guard is not active')
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '250s',
