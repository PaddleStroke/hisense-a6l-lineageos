...
CAPTURE = ROOT / 'capture-controls-user-v72'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V72 combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v72', 'suspend', 'idle']):
            raise RuntimeError('V72 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v72/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '6aa00cd02346a3f4312827bccb1db7df1a6ce3dd5147b721d2e43080ccc05ee4'):
