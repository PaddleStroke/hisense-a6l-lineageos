...
CAPTURE = ROOT / 'capture-diagnostic-install-user-v70'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v70', 'suspend', 'idle']):
            raise RuntimeError('V70 requires an active desktop idle/suspend inhibitor')
        run(['/usr/bin/python3', str(ROOT / 'Inspect-RecoveryImports-v4.py')])
        run(['/usr/bin/python3', str(ROOT / 'Inspect-DiagnosticRecovery-user-v70.py')], timeout=30)
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Write-LaptopDiagnosticRecovery-user-v70.py'), '--mode', 'install-diagnostic',
