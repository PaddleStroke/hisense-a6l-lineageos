...
CAPTURE = ROOT / 'capture-controls-user-v74c'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V74C combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v74c', 'suspend', 'idle']):
            raise RuntimeError('V74C requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v74c/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '24ede49b0f34b10f216617cb007c757e495fa2df1b6cd1ddd0afd63f9819da62'):
