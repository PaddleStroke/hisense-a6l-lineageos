...
PREV=ROOT/'firmware/extracted/recovery-rootedstock-20260921'
OUT=ROOT/'firmware/extracted/recovery-rootedstock-r2-20260921';OUT.mkdir(exist_ok=False)
...
patched=Path('/home/a6l/magisk/work2/new-boot.img')   # r2: the officially supported flow = patch the stock RECOVERY image
...
assert get(pargs,'cmdline')==get(rargs,'cmdline')
args=list(rargs)
args[args.index('--cmdline')+1]=get(rargs,'cmdline')+' androidboot.a6lslot=rec'   # marker: getprop ro.boot.a6lslot tells which slot really booted
...
report={'packaging_passed':True,'ready_to_flash':False,'reviewed_by_user':False,'candidate_sha256':sha(cand),'previous_sha256':prev,
 'kernel_sha256':sha(k),'ramdisk_sha256':sha(Path(get(pargs,'ramdisk')).read_bytes()),'command_line':get(args,'cmdline'),'layout':layout,
...
print('ROOTEDSTOCK_R2_PACKAGE_PASS',sha(cand),'body',len(b),flush=True)
