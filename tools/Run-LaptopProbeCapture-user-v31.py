...
CAPTURE = ROOT / 'capture-probe-serial-user-v31'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v31', 'suspend', 'idle']):
            raise RuntimeError('V31 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v31/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '0cb8efe9d42e3c1cd8114a6b6d17dde03a12ebb307a78e46a70419d34258d047'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '1880s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v31.py'),
