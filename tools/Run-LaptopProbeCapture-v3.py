...
CAPTURE = ROOT / 'capture-probe-serial-v3'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        install = json.loads((ROOT / 'capture-diagnostic-install-v3/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '9c368dba3dc9a3e70167114ea160f224750dc7a9c8d39b4bc72191dd51344ac3'):
        ...
        save()
        print('READY: diagnostic log capture is armed for four minutes. Follow the physical recovery boot instructions.', flush=True)
        boot_requested = True
