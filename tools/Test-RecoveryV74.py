#!/usr/bin/env python3
"""V74 candidate (agent v74img): regression-check against the captured ABL routines (generated from the V46 test)."""
import hashlib
...
def main():
    ...
    try:
        ...
        assert sha(base) == package['base_sha256']
        expected_base = read_fdt((ROOT / 'firmware/extracted/recovery-v71-candidate-20260921/base.dtb').read_bytes())
        for key, change in package['dt_changes'].items():
            node, prop = key.rsplit('/', 1)
            previous = expected_base.get(node, {}).get(prop)
            assert (None if previous is None else previous.hex()) == change['before']
            if change['after'] is None:
                del expected_base[node][prop]
            else:
                expected_base.setdefault(node, {})[prop] = bytes.fromhex(change['after'])
        for node, props in read_fdt(base).items():  # V71: property-less container nodes (e.g. pmic@68/regulators)
            if not props:
                expected_base.setdefault(node, {})
        ...
        assert sha(kernel) == package['kernel_sha256']
