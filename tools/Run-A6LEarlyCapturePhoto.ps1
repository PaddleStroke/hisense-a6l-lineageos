...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v4c-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v4.py';Capture='Run-LaptopProbeCapture-v4c.py';Restore='Run-LaptopDiagnosticRestore-v4.py'}[$Action]
