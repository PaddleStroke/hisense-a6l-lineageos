...
OLD = ROOT / 'firmware/extracted/recovery-probe-baseline-v34-20260917'
KERNEL = ROOT / 'firmware/extracted/baseline-6.19-kernel-20260917'
RAM = ROOT / 'firmware/extracted/baseline-6.19-ramdisk-20260917'
TEST = ROOT / 'firmware/extracted/baseline-6.19-probe-r2-20260917'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/baseline-6.19-smoke-20260917/report.json').read_text())['passed']
    ...
    assert sha(original) == '31e451b709ead424d93e4623922c9f6549f5229795853123be006611f7fa0706'
    assert json.loads((OLD / 'captured-abl-validation.json').read_text())['passed']
    OUT.mkdir(exist_ok=False)
    before = (OLD / 'base.dtb').read_bytes()
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V35 adds only regulator-allow-set-load to the two eMMC supplies. Same V34 6.19.10 kernel, RAM init/module, voltages, clocks and storage path. No fixed extra load votes.',
