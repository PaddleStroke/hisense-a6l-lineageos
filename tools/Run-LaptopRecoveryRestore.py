#!/usr/bin/env python3
"""Write-test the exact stock recovery in EDL and verify full readback/Android return."""
...
CAPTURE = ROOT / 'capture-recovery-restore-v1'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        run(['/usr/bin/python3', str(ROOT / 'Inspect-RecoveryRestoreImports-v1.py')])
        ...
        print('Enter the laptop password for the fixed STOCK RECOVERY write/readback test.', flush=True)
        ...
        print('Services paused. Checking GPT and recovery, writing the SAME stock recovery, then reading it back.', flush=True)
        boot_requested = True
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Write-LaptopStockRecovery-v1.py'),
                                     '--stock-write-test', '--output', str(CAPTURE / 'edl')], stdout=log, stderr=subprocess.STDOUT, timeout=140)
        report['worker_exit'] = result.returncode
        save()
        if result.returncode:
            raise RuntimeError('Stock recovery write test stopped; inspect the worker log before any retry or reset')
