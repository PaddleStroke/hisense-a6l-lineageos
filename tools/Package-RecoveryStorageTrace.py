...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-module-v19-20260916'
RAM = ROOT / 'firmware/extracted/storage-trace-ramdisk-v20-20260916'
TEST = ROOT / 'firmware/extracted/storage-trace-probe-v20-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-trace-smoke-v20-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '7449f5391f55e6e21df495ab4ea0cf4b8c1ec75e96b468827e66aa9458078258'
    ...
    assert sha(original) == '9c092eb41e8c894bff5aea6a1df442582cb4378d782e48d3883aafa1d04bed4e'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V19 DT and power settings preserved; opt-in bounded 250ms storage probe checkpoints, advance fork marker and 1s pre-load pause; no persistent mounts',
