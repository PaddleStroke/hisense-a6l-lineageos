...
ARCHIVE = ROOT / 'firmware/extracted/storage-commit-probe-v30-20260916'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-storage-commit-probe-v30-20260916')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/storage-commit-ramdisk-v30-20260916/ramdisk.cpio.gz'),
...
def main():
    ...
    image = (ROOT / 'firmware/extracted/storage-commit-kernel-v30-20260916/Image').read_bytes()
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 22 seconds; no physical USB emulation',
              ...
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/storage-commit-ramdisk-v30-20260916/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
