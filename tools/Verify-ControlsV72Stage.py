"""Verify V72 staged files only; never open USB or invoke a phone operation."""
import hashlib
import json
...
pins = json.loads((ROOT / 'v72-pins.json').read_text())
for name, digest in pins.items():
    assert not Path(name).is_absolute() and '..' not in Path(name).parts
    check(ROOT / name, digest)
manifest = json.loads((ROOT / 'diagnostic-user-v72-tools.json').read_text())
assert manifest['candidate_sha256'] == V72 and manifest['previous_sha256'] == V71
...
for name, digest in v38['files'].items():
    check(ROOT / name, digest)
check(ROOT / 'ram-staging/recovery-diagnostic-staged-usb-v72.img', V72)
check(ROOT / 'stock-recovery/restore-stock-recovery.img', STOCK)
assert json.loads((ROOT / 'v72/captured-abl-validation.json').read_text())['passed']
bundle = json.loads((ROOT / 'v72/bundle/manifest.json').read_text())['files']
for name, digest in bundle.items():
    assert not Path(name).is_absolute() and '..' not in Path(name).parts
    check(ROOT / 'v72/bundle' / name, digest)
print(json.dumps({'passed': True, 'pin_count': len(pins),
                  'verified': {'v38_tools': len(v38['files']), 'v72_tools': len(manifest['files']), 'v72_bundle': len(bundle)},
