"""Run the actual V32 C state machine against bounded fake SDHCI registers."""
from pathlib import Path
import re, subprocess, json
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'firmware/extracted/storage-state-logic-v32-20260917'
source=ROOT/'tools/a6l-cmd0-state-v32.c'
...
struct regulator { int enabled,voltage; };
struct mmc_host { struct { struct regulator *vmmc,*vqmmc; } supply; unsigned actual_clock; struct mmc_ios ios; };
struct sdhci_host; struct sdhci_ops { void (*dump_vendor_regs)(struct sdhci_host *); };
struct sdhci_host { struct sdhci_ops *ops; struct mmc_host *mmc; struct mmc_command *cmd; bool runtime_suspended; unsigned flags,clock,quirks,quirks2; int irq,lock; };
...
static int supply_queries, vendor_dumps;
...
static int regulator_is_enabled(struct regulator *s) { assert(!locked); supply_queries++; return s->enabled; }
static int regulator_get_voltage(struct regulator *s) { assert(!locked); supply_queries++; return s->voltage; }
static void vendor_dump(struct sdhci_host *h) { assert(locked); vendor_dumps++; }
...
'''
test=r'''
static struct sdhci_ops ops={vendor_dump}; static struct regulator vmmc={1,2944000},vqmmc={1,1800000};
...
static void reset(void) {
 ...
 host.mmc=&mmc; host.ops=&ops; mmc.supply.vmmc=&vmmc; mmc.supply.vqmmc=&vqmmc; supply_queries=vendor_dumps=0; host.cmd=&cmd; cmd.mrq=&mrq; opted=true; queued=false; locked=false;
...
static void issue(void) { start(); tick(); assert(signals==1 && commands==0); assert(supply_queries==4 && vendor_dumps==1); tick(); assert(commands==0 && queued); assert(strstr(logs,"PREISSUE_CHECKED")); tick(); assert(commands==1); }
...
binary=Path('/tmp/a6l-v32-stage-test')
