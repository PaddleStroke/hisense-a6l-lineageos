...
CAPTURE = ROOT / 'capture-probe-serial-user-v14'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v14/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'ca4c452b707524bac03d92626f27322ffd1327646a3eba3b8f735abf72c45971'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '250s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v14.py'),
