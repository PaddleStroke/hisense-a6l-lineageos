#!/usr/bin/env python3
"""One hash-pinned stock recovery transfer. Not a standalone flashing command.
...
def verify_stock_payload(payload):
    if not isinstance(payload, bytes) or len(payload) != RECOVERY_BYTES:
        raise ValueError('Stock recovery payload must be exactly 64 MiB of immutable bytes')
    digest = hashlib.sha256(payload).hexdigest()
    if digest != STOCK_HASH:
        raise ValueError('Payload differs from the verified stock recovery')
    return digest


def program_stock_recovery(fh, payload, report, save):
    """Program only the stock recovery; leave subsequent readback to the caller."""
    digest = verify_stock_payload(payload)
