#!/usr/bin/env python3
"""Package the scoped A6L boot-domain preservation diagnostic with the V7 device tree."""
import gzip
import hashlib
...
OUT = ROOT / 'firmware/extracted/recovery-probe-keep-boot-domains-20260915'
EARLY = ROOT / 'firmware/extracted/keep-boot-domains-probe-20260915'
...
def main():
    ...
    assert json.loads((ROOT / 'firmware/extracted/keep-boot-domains-source-20260915/function-tests.json').read_text())['passed']
    ...
    previous_dir = ROOT / 'firmware/extracted/recovery-probe-gpio-reserved-20260915'
    assert base == (previous_dir / 'base.dtb').read_bytes()
    assert overlay == (previous_dir / 'overlay.dtbo').read_bytes()
    before, after = read_fdt((OLD / 'base.dtb').read_bytes()), read_fdt(base)
    previous = read_fdt((ROOT / 'firmware/extracted/recovery-probe-pinctrl-20260915/base.dtb').read_bytes())
    assert previous.keys() == after.keys()
    changes = [(n, k) for n in previous for k in set(previous[n]) | set(after[n])
               if previous[n].get(k) != after[n].get(k)]
    assert changes == [('/soc@0/pinctrl@3100000', 'gpio-reserved-ranges')], changes
    assert after['/soc@0/pinctrl@3100000']['gpio-reserved-ranges'] == struct.pack('>II', 8, 4)
    ...
    for node, props in before.items():
        for key in set(props) | set(after[node]):
            if props.get(key) != after[node].get(key):
                if (node, key) == ('/soc@0/pinctrl@3100000', 'gpio-reserved-ranges'):
    ...
    report = dict(scope='Offline A6L boot-domain preservation candidate; no phone access',
                  ...
                  hardware_properties_preserved=False, device_tree_changes_from_v6=changes,
                  device_tree_matches_v7=True, modern_overlay_passed=True,
                  remaining=['Run captured ABL overlay, selection, fixup and packaging checks',
                             ...
                             'Early LCD console already demonstrated physically; new power-domain preservation image still requires a physical trial'],
