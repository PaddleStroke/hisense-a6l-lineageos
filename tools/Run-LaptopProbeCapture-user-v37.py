...
CAPTURE = ROOT / 'capture-probe-serial-user-v37'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v37', 'suspend', 'idle']):
            raise RuntimeError('V37 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v37/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '7de9a96229140b066aed2724ccf2cb14b1a09cc9e44a12bfb25ee4543761b8e9'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '1880s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v37.py'),
