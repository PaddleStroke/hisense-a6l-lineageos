"""Copy the V74 preparation to the laptop only when explicitly run; never launches anything on the phone.
...
manifest = json.loads((T / 'diagnostic-user-v74-tools.json').read_text())
assert manifest['candidate_sha256'] == V74
...
assert sha(image / 'recovery-diagnostic-unsigned.img') == V74
assert json.loads((image / 'captured-abl-validation.json').read_text())['passed']
bundle = ROOT / 'firmware/extracted/v74-attended-bundle-20260922'
...
files = {name: T / name for name in manifest['files'] if name != 'diagnostic-user-v74-tools.json'}
files['diagnostic-user-v74-tools.json'] = T / 'diagnostic-user-v74-tools.json'
files['Verify-ControlsV74Stage.py'] = T / 'Verify-ControlsV74Stage.py'
files['v74/captured-abl-validation.json'] = image / 'captured-abl-validation.json'
files['v74/bundle/manifest.json'] = bundle / 'manifest.json'
for name in bundle_files:
    files['v74/bundle/' + name] = bundle / name
files['ram-staging/recovery-diagnostic-staged-usb-v74.img'] = image / 'recovery-diagnostic-unsigned.img'
pins = {name: sha(path) for name, path in files.items()}
(T / 'v74-pins.json').write_text(json.dumps(pins, indent=2) + '\n')
files['v74-pins.json'] = T / 'v74-pins.json'

preflight = "import pathlib; r=pathlib.Path('" + REMOTE + "'); assert r.is_dir(); assert not (r/'capture-diagnostic-install-user-v74').exists(); assert not (r/'capture-diagnostic-restore-user-v74').exists(); print('STAGING_ONLY_READY')"
...
out = ROOT / 'research/recovery-v74-20260921'; out.mkdir(exist_ok=True)
result = run([SSH, '-F', CONFIG, 'a6l-laptop', 'python3 ' + REMOTE + '/Verify-ControlsV74Stage.py'])
(out / 'laptop-stage-verification.json').write_text(result)
inspect = run([SSH, '-F', CONFIG, 'a6l-laptop', 'python3 ' + REMOTE + '/Inspect-DiagnosticRecovery-user-v74.py'])
