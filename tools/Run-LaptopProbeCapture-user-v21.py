...
CAPTURE = ROOT / 'capture-probe-serial-user-v21'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v21/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'd4fa4f8b5b50e3c7b9e123ad5cde913d5e5000fee3535620af4e50773b13d407'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '340s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v21.py'),
