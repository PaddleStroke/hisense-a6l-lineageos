...
ARCHIVE = ROOT / 'firmware/extracted/visible-userspace-probe-20260915'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-visible-userspace-20260915')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/visible-userspace-ramdisk-20260915/ramdisk.cpio.gz'),
    ...
    try:
        with (p / 'console.log').open('wb') as log, (p / 'stderr.log').open('wb') as err:
            ...
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                ...
                while time.monotonic() < until:
                    ...
                    if (data.count(b'A6L_RAM_PROBE_ALIVE') >= 2 and data.count(b'A6L_DEFERRED_END') >= 2) or b'Kernel panic' in data:
                        break
        ...
        checks = dict(init_milestones_complete=all(pos >= 0 for pos in positions),
                      ...
                      alive=data.count(b'A6L_RAM_PROBE_ALIVE') >= 2,
...
def main():
    ...
    result = {'scope': 'Diskless QEMU test of revised RAM PID1 status via kmsg and read-only deferred-device snapshots; no A6L USB hardware emulation',
              'kernel_sha256': hashlib.sha256(adapted).hexdigest(),
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/visible-userspace-ramdisk-20260915/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
