"""Package V27: skip SDIO probing on the soldered eMMC; same V26 kernel/RAM."""
import gzip
import hashlib
...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-request-v26-20260916'
RAM = ROOT / 'firmware/extracted/storage-request-ramdisk-v26-20260916'
TEST = ROOT / 'firmware/extracted/storage-request-probe-v26-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-request-smoke-v26-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '410642b760aeb08b304014d74b83b60831a88cd924ee5a541d247ecf8266b494'
    ...
    assert sha(original) == 'fdf8ee6e41c49b598b6b03ffec00ab564bc87af0913122b58b2ad5cb3987dcfc'
    ...
    assert tree[node]['non-removable'] == b'' and tree[node]['bus-width'] == bytes.fromhex('00000008')
    assert 'no-sdio' not in tree[node] and 'no-mmc' not in tree[node]
    run('fdtput', '-t', 's', OUT / 'base.dtb', node, 'no-sdio')
    base = (OUT / 'base.dtb').read_bytes()
    expected = read_fdt(base_before)
    expected[node]['no-sdio'] = b''
    assert read_fdt(base) == expected, 'DT changed beyond one boolean'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V26 kernel, RAM, module, power settings and traces unchanged; add only no-sdio to eMMC controller DT',
              'dt_only_change': {'node': node, 'property': 'no-sdio', 'value_hex': ''},
