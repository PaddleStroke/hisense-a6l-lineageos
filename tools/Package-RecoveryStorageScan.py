...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-host-v23-20260916'
RAM = ROOT / 'firmware/extracted/storage-scan-ramdisk-v24-20260916'
TEST = ROOT / 'firmware/extracted/storage-scan-probe-v24-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-scan-smoke-v24-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '7335a99fd4fcffdce926654ea10a1599eb86b28f036505acc2e23637e6447ff5'
    ...
    assert sha(original) == 'fcc665755df0dfb3324f606701f8072dcdda3cef98227e416c593ba9ff113afa'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V23 DT/command line/init/module/power settings preserved; add16 card-scan checkpoints and extend shared trace bound to96',
