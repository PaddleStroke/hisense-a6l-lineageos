...
def main():
    ...
    compressed = payload[:len(payload) - len(dec.unused_data)]
    ...
    # position and mkbootimg's payload digest. All addresses and bootargs persist.
    old_header, new_header = bytearray(original[:4096]), bytearray(body[:4096])
    for start, stop in [(8, 12), (576, 608), (1632, 1644)]:
    ...
    report = dict(scope='Offline DT compatibility correction; no phone access',
                  packaging_passed=True, ready_to_flash=False, layout=layout,
                  kernel_and_ramdisk_unchanged=True, command_line=cmdline,
