...
"""
import argparse
from RecoveryTransitionBootM1 import verify_transition, RECOVERIES
...
from DiagnosticRecoveryProtocolBootM1 import check_program_xml, program_recovery, verify_payload, TARGETS

REGIONS = {
    'primary': (0, 1048576), 'tail': (125074122752, 22528),
    'recovery': (343932928, 67108864), 'devinfo': (253755392, 4096),
    'misc-bcb': (8757706752, 4096), 'vbmeta': (291811328, 65536),
}
RECOVERY_HASH = '55ad4747ea8d83a32edb09eafb01772a3cb170378331cdea3d45cd5a8cb34dbc'
...
def main():
    ...
    capture_name = 'capture-diagnostic-install-user-bootm1' if args.mode == 'install-diagnostic' else 'capture-diagnostic-restore-user-bootm1'
    ...
    try:
        ...
        stock = (home / 'stock-boot/restore-stock-boot.img').read_bytes()
        report['restore_payload_sha256'] = verify_payload(stock, 'restore-stock')
        payload = ((home / 'ram-staging/boot-magisk-staged-bootm1.img').read_bytes()
