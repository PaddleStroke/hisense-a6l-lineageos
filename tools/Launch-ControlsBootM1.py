"""Launch one verified BootM1 capture under the laptop's desktop sleep inhibitor."""
import json,os,subprocess
from pathlib import Path
root=Path(__file__).resolve().parent
assert not (root/'capture-controls-user-bootm1').exists()
...
with (root/'controls-bootm1-launch.log').open('x') as log:
    p=subprocess.Popen(['/usr/bin/gnome-session-inhibit','--app-id','A6L-bootm1-capture',
                        '--reason','A6L combined controls test','--inhibit','suspend:idle',
                        '/usr/bin/python3',str(root/'Run-LaptopControlsCapture-bootm1.py')],
