"""Assemble the V74 diagnostic recovery CANDIDATE (agent v74img, 23 Sep 2026). Offline only; nothing is flashed.

V74 = V71 with the DT rebuilt from the V68 base and the V71 overlays, picking up the 21-23 Sep proven fixes:
  - Bluetooth B2 (a6l-bluetooth-v74.dtso instead of a6l-bluetooth.dtso): /aliases serial1 = BLSP2 UART1 (c1af000);
  - e-ink E2 as in the repo since 21 Sep evening: bridge@0 xon-gpios = TLMM 61 active high (V73 panel driver takes it);
  - e-ink E3 (a6l-eink-v74.dtso, new): TPS65185 vcom regulator pinned to 2.40 V (min == max);
  - /chosen hisense,a6l-image = "v74" (NEW marker). hisense,a6l-controls stays "v71" ON PURPOSE: every bundle run.sh and the
    framework guard check it, so all V71/V74 payloads keep working unchanged on V74.
Kernel Image and ramdisk are byte-identical to V71. Every DT difference against the V71 base must be in the allowlist below.
Legacy V71 text follows:
...
OUT=ROOT/'firmware/extracted/recovery-v74-candidate-20260923'
PACK=Path('/home/a6l/android/a6l-lineage24/system/tools/mkbootimg')
V46=json.loads((OLD/'report.json').read_text())['candidate_sha256']  # predecessor = installed V71
assert V46=='417164b78c3bc7c43eba3039caab240194b5a69000b78737c50c3503f398bcf9'
EXTRA=[x for x in __import__('os').environ.get('A6L_V74_EXTRA','').split(',') if x]   # extra *-v74 overlays (label without a6l- and .dtso)
...
def main():
    ...
    for label in ['gpu','eink-pmic-reartouch','front-als','modem-wifi','display-native','eink-dsi','audio-internal','bluetooth-v74','eink-v74']+EXTRA:
    ...
    run('fdtput','-t','s',base,'/chosen','hisense,a6l-controls','v71');run('fdtput','-t','s',base,'/chosen','hisense,a6l-image','v74')
    ...
    new_nodes={n for n in after if n not in before};assert not new_nodes or EXTRA,new_nodes
    assert not [n for n in before if n not in after]
    bridge=find('dsi@c996000')+'/bridge@0';vcom=find('/pmic@68/regulators/vcom')
    touched=[bridge,vcom]
    allowed={(bridge,'xon-gpios'),(vcom,'regulator-min-microvolt'),(vcom,'regulator-max-microvolt'),('/aliases','serial1'),
             ('/chosen','hisense,a6l-bluetooth'),('/chosen','hisense,a6l-image'),('/chosen','hisense,a6l-eink-v74')}
    ...
    for node in before.keys()|after.keys():
        for prop in before.get(node,{}).keys()|after.get(node,{}).keys():
            ...
            ok=(node,prop) in allowed or (EXTRA and (node in new_nodes or node=='/__symbols__' or node=='/chosen'))
            assert ok,(node,prop)
    ...
    # V74 checks
    tlmm=struct.unpack('>I',after[find('/pinctrl@3100000')]['phandle'])[0]
    assert struct.unpack('>III',after[bridge]['xon-gpios'])==(tlmm,61,0),'xon-gpios must be TLMM 61 active high'
    assert after[vcom]['regulator-min-microvolt']==after[vcom]['regulator-max-microvolt']==struct.pack('>I',2400000)
    assert after['/aliases']['serial1']==b'/soc@0/serial@c1af000\0' and after['/aliases']['serial0']==before['/aliases']['serial0']
    uart=after['/soc@0/serial@c1af000'];assert uart['status']==b'okay\0' and after['/soc@0/serial@c1af000/bluetooth']['compatible']==b'qcom,wcn3990-bt\0'
    assert after['/chosen']['hisense,a6l-controls']==b'v71\0' and after['/chosen']['hisense,a6l-image']==b'v74\0' and after['/chosen']['hisense,a6l-bluetooth']==b'b2\0'
    pm=after[find('/pmic@68')]
    for k,g in [('enable-gpios',35),('vcom-ctrl-gpios',3),('wakeup-gpios',80),('pwr-good-gpios',0)]:assert struct.unpack('>III',pm[k])==(tlmm,g,0),k
    assert pm==before[find('/pmic@68')]
    ...
    print('phandle-shift properties',shifts,flush=True);assert shifts==0 or EXTRA,'unexpected phandle shifts'
    ...
    report={'packaging_passed':True,'ready_to_flash':False,'reviewed_by_user':False,'candidate_sha256':sha(candidate),'previous_sha256':V46,
            ...
            'scope':'V71 DT rebuilt with bluetooth B2 (serial1 alias), e-ink XON gpio61 + VCOM 2.40 V constraint, image marker v74 (controls marker stays v71); overlays: '+','.join(['bluetooth-v74','eink-v74']+EXTRA)+'. Same kernel and ramdisk as V71. Not booted anywhere.'}
    (OUT/'report.json').write_text(json.dumps(report,indent=2)+'\n')
    print('V74_PACKAGE_PASS',sha(candidate),'properties',len(changes),flush=True)
