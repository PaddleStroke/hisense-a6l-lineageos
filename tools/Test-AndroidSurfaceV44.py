...
OUT=Path(f'/home/a6l/kernel/surface-v44-r{n}');OUT.mkdir(exist_ok=False)
ARCH=ROOT/f'firmware/extracted/android-surface-v44-20260917-r{n}';ARCH.mkdir(exist_ok=False)
...
put('bin/a6l_graphics_services',P/'system/bin/a6l_surface_services',0o755)
...
seeds=[('system','bin/servicemanager'),('system','bin/hwservicemanager'),('system','bin/a6l_surface_client'),('system','bin/surfaceflinger'),('system','lib64/libEGL_angle.so'),('system','lib64/libGLESv1_CM_angle.so'),('system','lib64/libGLESv2_angle.so'),('vendor','lib64/hw/vulkan.pastel.so'),
...
for name in ['surface_services.c','surface_client.cpp','private_properties.h']:
...
for log in Path('/home/a6l/logs').glob('build-android-surface-v44*.log'):(ARCH/log.name).write_bytes(log.read_bytes())
script=OUT/'qemu.sh';script.write_text('''#!/system/bin/sh
/system/bin/toybox mknod /dev/v44-console c 204 64
exec > /dev/v44-console 2>&1
echo A6L_QEMU_GRAPHICS_START
/system/bin/toybox mkdir -p /tmp/a6l-v44
/system/bin/toybox cp -R /v44/. /tmp/a6l-v44/
/system/bin/toybox insmod /tmp/a6l-v44/a6l_simplefb.ko
echo A6L_QEMU_GRAPHICS_MODULE_EXIT=$?
ready_before=$(/system/bin/getprop servicemanager.ready)
(
  while [ ! -f /tmp/a6l-v44/root/logs/client.log ]; do /system/bin/toybox sleep 0.1; done
  /system/bin/toybox tail -n +1 -f /tmp/a6l-v44/root/logs/client.log
) &
tail_pid=$!
/tmp/a6l-v44/bin/a6l_graphics_services
probe_exit=$?
kill "$tail_pid"
...
for f in /tmp/a6l-v44/root/logs/*; do
 echo LOGFILE:$f
...
on early-init
    start graphicstest
service graphicstest /system/bin/sh /v44/qemu.sh
    user root
    group root
    disabled
    oneshot
...
dirs={'/v44'}
for name in files:dirs.update('/v44/'+str(p) for p in Path(name).parents if str(p)!='.')
lines += [f'dir {p} 0755 0 0' for p in sorted(dirs)]
lines += [f'file /v44/{name} {payload/name} {i["mode"]:04o} 0 0' for name,i in files.items()]
lines += [f'file /v44/qemu.sh {script} 0755 0 0']
...
sock=Path(f'/tmp/v44-{os.getpid()}.sock')
...
with (OUT/'console.log').open('wb') as f:
    ...
    try:
        ...
        with socket.socket(socket.AF_UNIX,socket.SOCK_STREAM) as conn:
            ...
            qmp('qmp_capabilities');deadline=time.monotonic()+150
            while time.monotonic()<deadline:
                data=(OUT/'console.log').read_bytes()
                if not sampled and b'A6L_SURFACE_VISIBLE' in data:
...
data=(OUT/'console.log').read_bytes();checks={
 ...
 'services':b'A6L_SURFACE_SERVICES_PASS client=1 services_alive=5 private_properties=1 namespace_cleanup=1' in data,
 'ready_property_restored':b'A6L_QEMU_READY_RESTORED=1' in data,
 'no_panic':b'Kernel panic' not in data}
checks['present']=b'A6L_SURFACE_PASS real_surfaceflinger=1 layers=4 transaction=checked' in data
...
if sampled:
    ...
    top=struct.pack('<I',0xffff0000)*540+struct.pack('<I',0xff00ff00)*540
