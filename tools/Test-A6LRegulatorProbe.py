...
ARCHIVE = ROOT / 'firmware/extracted/regulator-probe-20260916'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-regulator-probe-20260916')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/regulator-ramdisk-20260916/ramdisk.cpio.gz'),
...
def main():
    ...
    result = {'scope': 'Diskless QEMU test of revised RAM PID1 status via kmsg and read-only deferred-device snapshots; no A6L USB hardware emulation',
              ...
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/regulator-ramdisk-20260916/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
