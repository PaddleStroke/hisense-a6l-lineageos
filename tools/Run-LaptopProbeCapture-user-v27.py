...
CAPTURE = ROOT / 'capture-probe-serial-user-v27'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v27', 'suspend', 'idle']):
            raise RuntimeError('V27 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v27/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'a52505613120c8635d67947b76f937b6615b96665a408e75a119843183fcbdf6'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '1880s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v27.py'),
