...
CAPTURE = ROOT / 'capture-diagnostic-install-user-v73'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        run(['/usr/bin/python3', str(ROOT / 'Inspect-DiagnosticRecovery-user-v73.py')], timeout=30)
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Write-LaptopDiagnosticRecovery-user-v73.py'), '--mode', 'install-diagnostic',
