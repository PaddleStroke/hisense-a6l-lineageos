...
CAPTURE = ROOT / 'capture-probe-serial-user-v26-repeat'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v26', 'suspend', 'idle']):
            raise RuntimeError('V26 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v26/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'fdf8ee6e41c49b598b6b03ffec00ab564bc87af0913122b58b2ad5cb3987dcfc'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '410s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v26-repeat.py'),
