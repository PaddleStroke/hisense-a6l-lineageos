...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-nosdio-v27-20260916'
RAM = ROOT / 'firmware/extracted/storage-hold-ramdisk-v28-20260916'
TEST = ROOT / 'firmware/extracted/storage-hold-probe-v28-20260916'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-hold-smoke-v28-20260916/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '610940b3868a7f4b1d83e25950a76f9e420a850060d1798f7f67aae1bb59cce4'
    ...
    assert sha(original) == 'a52505613120c8635d67947b76f937b6615b96665a408e75a119843183fcbdf6'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V27 hardware DT and RAM init preserved; hold CMD0 immediately before command register write, cancel timer, leave request pending for RAM logging',
