"""V33: omit only the optional eMMC GCC block-reset DT property; V32 binaries unchanged."""
import gzip
import hashlib
...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-state-v32-20260917'
RAM = ROOT / 'firmware/extracted/storage-state-ramdisk-v32-20260917'
TEST = ROOT / 'firmware/extracted/storage-state-probe-v32-20260917'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/storage-state-smoke-v32-20260917/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256'] == '01e882200d5c99b1199d15697721de50dc0114e3465f62945ead2b68a5ee2c12'
    ...
    assert sha(original) == '47a31604c4d41743406201c85ae5ea7eb68999cf3bb0a41756c3a5bf224f9278'
    assert json.loads((OLD / 'captured-abl-validation.json').read_text())['passed']
    OUT.mkdir(exist_ok=False)
    base_before = (OLD / 'base.dtb').read_bytes()
    (OUT / 'base.dtb').write_bytes(base_before)
    node = '/soc@0/mmc@c0c4000'
    tree = read_fdt(base_before)
    assert tree[node]['resets'] == bytes.fromhex('000000200000000a')
    assert tree[node]['status'] == b'okay\0'
    assert tree[node]['non-removable'] == b'' and tree[node]['no-sdio'] == b''
    assert tree[node]['bus-width'] == bytes.fromhex('00000008')
    run('fdtput', '-d', OUT / 'base.dtb', node, 'resets')
    base = (OUT / 'base.dtb').read_bytes()
    expected = read_fdt(base_before)
    del expected[node]['resets']
    assert read_fdt(base) == expected, 'DT changed beyond optional eMMC reset removal'
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'V33 removes only optional eMMC GCC reset property; exact V32 kernel, RAM, module, command sequence, state snapshot and automatic branches',
              'dt_only_change': {'node': node, 'removed_property': 'resets', 'previous_hex': tree[node]['resets'].hex()},
              'base_before_sha256': sha(base_before), 'base_after_sha256': sha(base),
