"""Copy the V73 preparation to the laptop only when explicitly run; never launches anything on the phone.
...
manifest = json.loads((T / 'diagnostic-user-v73-tools.json').read_text())
assert manifest['candidate_sha256'] == V73
...
assert sha(image / 'recovery-diagnostic-unsigned.img') == V73
assert json.loads((image / 'captured-abl-validation.json').read_text())['passed']
bundle = ROOT / 'firmware/extracted/v73-attended-bundle-20260922'
...
files = {name: T / name for name in manifest['files'] if name != 'diagnostic-user-v73-tools.json'}
files['diagnostic-user-v73-tools.json'] = T / 'diagnostic-user-v73-tools.json'
files['Verify-ControlsV73Stage.py'] = T / 'Verify-ControlsV73Stage.py'
files['v73/captured-abl-validation.json'] = image / 'captured-abl-validation.json'
files['v73/bundle/manifest.json'] = bundle / 'manifest.json'
for name in bundle_files:
    files['v73/bundle/' + name] = bundle / name
files['ram-staging/recovery-diagnostic-staged-usb-v73.img'] = image / 'recovery-diagnostic-unsigned.img'
pins = {name: sha(path) for name, path in files.items()}
(T / 'v73-pins.json').write_text(json.dumps(pins, indent=2) + '\n')
files['v73-pins.json'] = T / 'v73-pins.json'

preflight = "import pathlib; r=pathlib.Path('" + REMOTE + "'); assert r.is_dir(); assert not (r/'capture-diagnostic-install-user-v73').exists(); assert not (r/'capture-diagnostic-restore-user-v73').exists(); print('STAGING_ONLY_READY')"
...
out = ROOT / 'research/recovery-v73-20260921'; out.mkdir(exist_ok=True)
result = run([SSH, '-F', CONFIG, 'a6l-laptop', 'python3 ' + REMOTE + '/Verify-ControlsV73Stage.py'])
(out / 'laptop-stage-verification.json').write_text(result)
inspect = run([SSH, '-F', CONFIG, 'a6l-laptop', 'python3 ' + REMOTE + '/Inspect-DiagnosticRecovery-user-v73.py'])
