...
CAPTURE = ROOT / 'capture-input-user-v47'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test V47 RAM input on installed V46 recovery with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v47', 'suspend', 'idle']):
            raise RuntimeError('V47 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v46/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'ce3727dda592065becb883ef3fe663cb3579290edb841854e01f4fc49d02a3c6'):
