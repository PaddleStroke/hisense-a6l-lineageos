...
CAPTURE = ROOT / 'capture-stock-restore-after-windows-v1'
SERIAL = '1e529013'
...
def main():
    ...
    assert hashlib.sha256((ROOT / 'Write-LaptopStockRestoreAfterWindows-v1.py').read_bytes()).hexdigest() == 'b1536f93ae66b041c945563d3577beb7de4d06a1cd48325fbc9e4ff678ab61d5'
    ...
    try:
        ...
        run(['/usr/bin/python3', str(ROOT / 'Inspect-DiagnosticRecovery-v12.py')], timeout=30)
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Write-LaptopStockRestoreAfterWindows-v1.py'),
