...
ARCHIVE = ROOT / 'firmware/extracted/earlycon-trace-probe-v3-20260915'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-earlycon-trace-v3-20260915')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/diagnostic-ramdisk-20260914/ramdisk.cpio.gz'),
            '-append', 'earlycon=a6lfb keep_bootcon console=ttyAMA0,115200 loglevel=8 panic=0 initcall_debug']
    ...
    try:
        with (p / 'console.log').open('wb') as log, (p / 'stderr.log').open('wb') as err:
            ...
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                ...
                while time.monotonic() < until:
                    ...
                    if data.count(b'A6L_RAM_PROBE_ALIVE') >= 2 or b'Kernel panic' in data:
                        break
        ...
        checks = dict(probe_entry_trace=b'A6L probe begin' in data,
...
def main():
    ...
    result = {'scope': 'Diskless QEMU CPU/RAM tests; does not emulate A6L scanout, USB or PMIC',
              'kernel_sha256': hashlib.sha256(adapted).hexdigest(), 'cases': {}}
    try:
        for name, board, size in [('enabled', 'hisense,hlte730t', 0x23ff000),
                                  ('wrong-board', 'linux,dummy-virt', 0x23ff000),
                                  ('wrong-reservation', 'hisense,hlte730t', 0x23fe000)]:
