#!/usr/bin/env python3
"""Capture a physical stock recovery control boot and wait for manual return to original Android."""
...
CAPTURE = ROOT / 'capture-stock-control-boot-v1'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Read-only diagnostic USB capture; no phone reboot or write command',
    ...
    try:
        install = json.loads((ROOT / 'capture-stock-control-install-v1/edl/report.json').read_text())
        if not (install.get('mode') == 'install-control' and install.get('readback_verified')
                and install.get('power_acknowledged') and install.get('power_action') == 'off'
                and not install.get('error')):
            raise RuntimeError('Verified diagnostic installation/poweroff evidence is required')
        if Path('/sys/bus/usb/devices/3-2').exists():
            raise RuntimeError('Expected powered-off spare without an enumerated USB device')
        print('Enter the laptop password for read-only stock-control USB observation.', flush=True)
        ...
        save()
        print('READY: stock-control USB observation is armed for four minutes. Follow the physical recovery boot instructions.', flush=True)
        boot_requested = True
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '250s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-StockControl-v1.py'),
        ...
        if boot_requested:
            ...
            while time.monotonic() < deadline:
                if not usb('05c6', '9008'):
                    ...
                    print('EDL appeared during the stock recovery control boot; keep USB connected for inspection.', flush=True)
                save()
                time.sleep(2)
        restored = True
