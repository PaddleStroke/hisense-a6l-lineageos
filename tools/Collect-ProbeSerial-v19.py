...
try:
    with (args.output / 'console.bin').open('wb') as log:
        while time.monotonic() < deadline and (boot_deadline is None or time.monotonic() < boot_deadline):
            ...
            if (boot_deadline is None and last_usb and last_usb.get("idVendor") == "18d1"
                ...
                report["post_selection_capture_seconds"] = 45
            if state != last_usb:
            ...
            if chunk:
                ...
                if (report['ready_seen'] and b'A6L_SERIAL_WRITE_END' in payload
                        and any(int(v) >= 32 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+)', payload))):
                    break
    ...
    report['staged_capture_complete'] = (b'A6L_SERIAL_WRITE_END' in payload and any(int(v) >= 32 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+)', payload)))
