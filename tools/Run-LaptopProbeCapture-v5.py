...
CAPTURE = ROOT / 'capture-probe-serial-v5'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        install = json.loads((ROOT / 'capture-diagnostic-install-v5/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '886baf8c66f85b199e3e0d8449731e5e98488d7c38641e7942f31ce54a216c38'):
