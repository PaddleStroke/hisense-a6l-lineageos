...
/* Regulator queries may sleep: execute once before taking host->lock.
 * Values are framework reports, not a physical voltage measurement.
 */
static void a6l_supply_snapshot(struct device *dev, struct regulator *supply,
				const char *name)
{
	int enabled, voltage;

	if (IS_ERR_OR_NULL(supply)) {
		dev_info(dev, "A6L_SUPPLY_V32 %s unavailable\n", name);
		return;
	}
	enabled = regulator_is_enabled(supply);
	voltage = regulator_get_voltage(supply);
	dev_info(dev, "A6L_SUPPLY_V32 %s enabled=%d voltage_uv=%d\n",
		 name, enabled, voltage);
}
...
static void a6l_cmd0_work(struct work_struct *work)
{
	...
	if (a6l_cmd0_step == 0 && READ_ONCE(host->cmd) == a6l_cmd0_cmd &&
	    !READ_ONCE(host->runtime_suspended) &&
	    !(READ_ONCE(host->flags) & SDHCI_DEVICE_DEAD)) {
		a6l_supply_snapshot(dev, host->mmc->supply.vmmc, "vmmc");
		a6l_supply_snapshot(dev, host->mmc->supply.vqmmc, "vqmmc");
	}
	...
	case 0:
		dev_info(dev, "A6L_STATE_V32_BEGIN flags=%08x hostctl=%02x hostctl2=%04x transfer=%04x block_size=%04x block_count=%04x caps=%08x\n",
			 host->flags, sdhci_readb(host, SDHCI_HOST_CONTROL),
		...
		dev_info(dev, "A6L_STATE_V32_END next=existing_baseline\n");
	...
	case 1:
		...
		break;
	case 2:
		/* V32: deliberately no new controller reads between the delivered
			  ...
			  u16 command)
{
	...
	dev_info(mmc_dev(host->mmc), "A6L_CMD0_STAGED_V32 next=baseline delay_ms=4000\n");
	schedule_delayed_work(&a6l_cmd0_delayed, msecs_to_jiffies(4000));
	return true;
}
