...
CAPTURE = ROOT / 'capture-controls-user-v71'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V71 combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v71', 'suspend', 'idle']):
            raise RuntimeError('V71 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v71/edl/report.json').read_text())
