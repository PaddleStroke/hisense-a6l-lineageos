...
CAPTURE = ROOT / 'capture-controls-user-v45'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V45 combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v45', 'suspend', 'idle']):
            raise RuntimeError('V45 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v45/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'aae86f0da6f15a0c1f6e425e88806edb165289c459462b232f0b8773f5e4ac14'):
