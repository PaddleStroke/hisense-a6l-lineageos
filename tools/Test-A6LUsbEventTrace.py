...
ARCHIVE = ROOT / 'firmware/extracted/staged-usb-probe-v16-20260916'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-staged-usb-probe-v16-20260916')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/staged-usb-ramdisk-v16-20260916/ramdisk.cpio.gz'),
    ...
    try:
        ...
        checks = dict(init_milestones_complete=all(pos >= 0 for pos in positions),
                      ...
                      staged_schedule=b'A6L_STAGED_USB_V16 config_after=0 bind_after=4 connect_after=8 serial_after=12 heartbeat=2' in data,
...
def main():
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 22 seconds; no physical USB emulation',
              ...
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/staged-usb-ramdisk-v16-20260916/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
