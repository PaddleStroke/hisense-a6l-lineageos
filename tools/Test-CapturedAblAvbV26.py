...
from unicorn import Uc, UC_ARCH_ARM64, UC_MODE_ARM, UC_HOOK_CODE, UC_QUERY_TIMEOUT
...
def verify(partitions, trusted_key=False):
    ...
    result = {}
    try:
        cpu.emu_start(0x8948, 0x1800fff0, timeout=120000000, count=30000000)
        result.update(returned=cpu.reg_read(UC_ARM64_REG_PC) == 0x1800fff0,
                      result=cpu.reg_read(UC_ARM64_REG_X0), pc=hex(cpu.reg_read(UC_ARM64_REG_PC)),
                      wall_timeout_reached=bool(cpu.query(UC_QUERY_TIMEOUT)))
