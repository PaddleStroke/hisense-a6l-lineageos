...
class WriterChecks(unittest.TestCase):
    ...
    def test_cross_mode_payload_and_unrecognized_modes_rejected(self):
        ...
        directory = Path(__file__).resolve().parents[1] / 'firmware/extracted/recovery-probe-visible-userspace-20260915'
        if not directory.exists():
            directory = Path(__file__).resolve().parent
            files = [('install-diagnostic', directory / 'ram-staging/recovery-diagnostic-visible-userspace.img'),
