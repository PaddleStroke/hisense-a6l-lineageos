...
CAPTURE = ROOT / 'capture-diagnostic-install-user-bootm1'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        run(['/usr/bin/python3', str(ROOT / 'Inspect-DiagnosticRecovery-user-bootm1.py')], timeout=30)
        ...
        print('Services paused. Verifying spare/GPT/known predecessor/exact boot message, installing diagnostic recovery, reading back, then powering off.', flush=True)
        boot_requested = True
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Write-LaptopDiagnosticRecovery-user-bootm1.py'), '--mode', 'install-diagnostic',
