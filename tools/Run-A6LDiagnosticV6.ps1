...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v6-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v6.py';Capture='Run-LaptopProbeCapture-v6.py';Restore='Run-LaptopDiagnosticRestore-v6.py'}[$Action]
