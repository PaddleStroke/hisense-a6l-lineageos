...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v11-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v11.py';Capture='Run-LaptopProbeCapture-v11.py';Restore='Run-LaptopDiagnosticRestore-v11.py'}[$Action]
