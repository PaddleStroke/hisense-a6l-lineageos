...
spec=importlib.util.spec_from_file_location('runner',Path(__file__).with_name('Run-ControlsV46.py'));runner=importlib.util.module_from_spec(spec);spec.loader.exec_module(runner)
class Guards(unittest.TestCase):
    def reject(self,identity=None,marker='v46\x00',mounts='tmpfs /tmp tmpfs rw 0 0\n',corrupt=False):
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder);payload=root/'controls-v46/payload';payload.mkdir(parents=True)
            ...
            with patch.object(runner,'ROOT',root),patch.object(runner,'PACKAGE',payload.parent),patch.object(runner.subprocess,'run',side_effect=fake),patch('sys.argv',['Run-ControlsV46.py','vibration']),contextlib.redirect_stdout(io.StringIO()):
