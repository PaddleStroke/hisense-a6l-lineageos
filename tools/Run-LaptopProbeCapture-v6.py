...
CAPTURE = ROOT / 'capture-probe-serial-v6'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        install = json.loads((ROOT / 'capture-diagnostic-install-v6/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'ef81c365f79648b66257d2ce55f2553b829629157874c629d0bad7899abfb7e2'):
