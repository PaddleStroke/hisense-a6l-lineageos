#!/usr/bin/env python3
"""Verify stock recovery/empty boot message in EDL and test the audited poweroff."""
...
CAPTURE = ROOT / 'capture-recovery-poweroff-v1'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Fixed reads and poweroff only; no image or partition write',
    ...
    try:
        ...
        print('Enter the laptop password for fixed stock reads and POWER OFF. No image will be written.', flush=True)
        ...
        print('Services paused. Checking GPT, recovery and empty boot message, then requesting poweroff.', flush=True)
        boot_requested = True
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Read-LaptopRecoveryPoweroff-v1.py'),
                                     '--output', str(CAPTURE / 'edl')], stdout=log, stderr=subprocess.STDOUT, timeout=140)
        report['worker_exit'] = result.returncode
        save()
        if result.returncode:
            raise RuntimeError('Poweroff test stopped; inspect the worker log')
        print('Read checks and poweroff acknowledgment passed. Waiting for EDL USB to disappear.', flush=True)
        exit_code = 0
