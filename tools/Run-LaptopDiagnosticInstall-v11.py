...
CAPTURE = ROOT / 'capture-diagnostic-install-v11'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        run(['/usr/bin/python3', str(ROOT / 'Inspect-DiagnosticRecovery-v11.py')], timeout=30)
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Write-LaptopDiagnosticRecovery-v11.py'), '--mode', 'install-diagnostic',
