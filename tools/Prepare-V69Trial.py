...
V46='2448b101eb780b1630cc6fd7181315da50211de2504b08cd6dad9e34d44a7520'  # here: the V68 tools' candidate
V45='ce3727dda592065becb883ef3fe663cb3579290edb841854e01f4fc49d02a3c6'  # here: the V68 tools' predecessor
...
pins=json.loads((T/'diagnostic-user-v68-tools.json').read_text())['files']
...
def v68(text):
    ...
    text=text.replace('verified-v46','@@LABEL@@').replace('verified V46 predecessor','@@PRED@@')
    text=text.replace('recovery-v68-candidate-20260920','recovery-@@C@@-candidate-20260921')
    return text.replace('V68','V69').replace('v68','v69').replace('@@C@@','v69').replace('@@LABEL@@','verified-v68').replace('@@PRED@@','verified V68 predecessor')
def put(name,text):
    p=T/name;p.write_text(text);py_compile.compile(str(p),doraise=True);manifest['files'][name]=digest(p)
for old in [n for n in pins if n!='diagnostic-user-v68-tools.json']:
    put(old.replace('V68','V69').replace('v68','v69'),v68(source(old)))
put('diagnostic-user-v69-tools.json',json.dumps(manifest,indent=2)+'\n')
