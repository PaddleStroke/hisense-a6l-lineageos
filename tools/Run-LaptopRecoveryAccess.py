#!/usr/bin/env python3
"""Coordinate the first fixed EDL read on the laptop and verify Android return."""
...
CAPTURE = ROOT / 'capture-recovery-access-v4'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'EDL fixed reads only; no partition write, then normal Android return',
    ...
    try:
        ...
        print('Enter the laptop password for this bounded recovery-access check.', flush=True)
        ...
        for unit in ['fwupd.service', 'ModemManager.service']:
            ...
            if 'LoadState=masked' in state and 'ActiveState=inactive' in state:
                # The first attempt can leave our masks while waiting for a
                # physical restart. Adopt only that completed attempt's masks.
                previous = json.loads((ROOT / 'capture-recovery-access/session.json').read_text())
                worker = json.loads((ROOT / 'capture-recovery-access/edl/report.json').read_text())
                expected_mask = ['/usr/bin/sudo', '-n', '/usr/bin/systemctl', 'mask', '--runtime', '--now', unit]
                initial_show = ['/usr/bin/systemctl', 'show', unit, '-p', 'LoadState', '-p', 'ActiveState']
                states = [c for c in previous['commands'] if c['argv'] == initial_show]
                if (not previous.get('finished_utc') or previous.get('worker_exit') != 1
                        or worker.get('operations') != [] or not worker.get('error', '').startswith('Usage:')
                        or not any(c['argv'] == expected_mask and c.get('exit') == 0 for c in previous['commands'])
                        or not states or 'LoadState=loaded' not in states[0]['stdout']
                        or not override.is_symlink() or os.readlink(override) != '/dev/null'):
                    raise RuntimeError('Cannot attribute the existing service mask to our stopped read-only attempt')
                masked.append((unit, 'ActiveState=active' in states[0]['stdout']))
                report.setdefault('adopted_masks_from_first_attempt', []).append(unit)
                save()
                continue
        ...
        print('Services paused. Reading recovery in emergency mode; no image will be written.', flush=True)
        boot_requested = True
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/sudo', '-n', '/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '130s',
                                     '/usr/bin/python3', str(ROOT / 'Read-LaptopRecovery-v4.py'),
                                     '--output', str(CAPTURE / 'edl')], stdout=log, stderr=subprocess.STDOUT, timeout=140)
        report['worker_exit'] = result.returncode
        save()
        if result.returncode:
            raise RuntimeError('Recovery read stopped; inspect the worker log before retrying')
        print('Recovery read verified. Waiting for the original Android installation.', flush=True)
        exit_code = 0
        ...
        if boot_requested:
            ...
            while time.monotonic() < deadline:
                if not usb('05c6', '9008'):
                    ...
                    print('EDL check stopped without writes. Hold only Power about 20 seconds to return to Android.', flush=True)
                save()
                time.sleep(2)
        restored = True
