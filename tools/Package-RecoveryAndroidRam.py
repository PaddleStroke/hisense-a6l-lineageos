...
OLD = ROOT / 'firmware/extracted/recovery-probe-storage-read-v37-20260917'
KERNEL = ROOT / 'firmware/extracted/baseline-7.2-kernel-20260917'
RAM = ROOT / 'firmware/extracted/android-ram-v38-20260917-r2'
TEST = ROOT / 'firmware/extracted/android-ram-probe-v38-20260917-r2'
...
def main():
    ...
    assert test['passed']
    assert json.loads((ROOT / 'firmware/extracted/android-init-module-20260917/report.json').read_text())['passed']
    kernel = (TEST / 'Image.a6l-offset').read_bytes()
    assert sha(kernel) == test['kernel_sha256']
    ...
    assert json.loads((RAM / 'qemu-report.json').read_text())['passed']
    original = (OLD / 'recovery-diagnostic-unsigned.img').read_bytes()
    assert sha(original) == '7de9a96229140b066aed2724ccf2cb14b1a09cc9e44a12bfb25ee4543761b8e9'
    ...
    cmdline = old_cmdline + ' androidboot.selinux=permissive androidboot.init_rc=/system/etc/init/hw/init.rc'
