...
ARCHIVE = ROOT / 'firmware/extracted/android-ram-probe-v38-20260917-r2'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-android-ram-probe-v38-20260917-r2')
...
def test(name, compatible, reserved_bytes):
    ...
    # With a deliberately short framebuffer reservation the remaining sampled
    # memory must be separately reserved, or normal RAM allocation can change it.
    guard = ('test-guard@9d401000 { reg = <0 0x9d401000 0 0x23fe000>; no-map; };'
             if reserved_bytes == 0x1000 else '')
    patch = f'''/dts-v1/;
/plugin/;
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/android-ram-v38-20260917-r2/ramdisk.cpio.gz'),
            '-append', 'earlycon=a6lfb keep_bootcon console=ttyAMA0,115200 loglevel=8 panic=0 androidboot.selinux=permissive androidboot.init_rc=/system/etc/init/hw/init.rc initcall_debug pd_ignore_unused a6l_manual_usb=1 a6l_usb_trace=1 a6l_storage_trace=1']
    ...
    try:
        with (p / 'console.log').open('wb') as log, (p / 'stderr.log').open('wb') as err:
            ...
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                ...
                while time.monotonic() < until:
                    ...
                    if (any(int(v) >= 22 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+) ', data))) or b'Kernel panic' in data:
                        break
        ...
        checks = dict(hash_selftest=b'A6L_STORAGE_HASH_SELFTEST success=1' in data,
                      no_read_before_usb=b'A6L_STORAGE_READ_FORK_BEGIN' not in data,
                      pid1_ready=b'A6L_RAM_PROBE_READY' in data,
                      alive=any(int(v) >= 22 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+) ', data)),
                      staged_schedule=b'A6L_STAGED_STORAGE_V20 config_after=0 bind_after=4 connect_after=8 serial_after=12 module_after=20 heartbeat=2' in data,
                      ...
                      no_kmsg_rate_drops=all(b'kauditd_printk_skb:' in line for line in data.splitlines() if b'callbacks suppressed' in line),
                      android_second_stage=b'init second stage started!' in data,
                      android_service=b'A6L_ANDROID_SERVICE_START' in data and b'A6L_ANDROID_PROPERTY ramdiag=v38' in data,
                      android_adbd=b'A6L_ANDROID_ADBD state=running' in data,
        ...
        # SELinux audit has its own rate limiter; diagnostic /dev/kmsg drops remain a failure.
        result['audit_suppression_lines'] = [v.decode(errors='replace') for v in data.splitlines() if b'kauditd_printk_skb:' in v]
        result.update(pixel_counts={hex(k): v for k, v in counts.items() if k in (0, 0xff000000, 0xffffffff)},
                      distinct_pixel_values=len(counts), checks=checks,
                      passed=all(checks.values()))
        assert result['passed'], {k:v for k,v in checks.items() if not v}
...
def main():
    ...
    image = (ROOT / 'firmware/extracted/android-init-kernel-20260917/Image').read_bytes()
    assert b'A6L_USB_CONNECT_HELD' in image
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 22 seconds; no physical USB emulation',
              'kernel_sha256': hashlib.sha256(adapted).hexdigest(),
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/android-ram-v38-20260917-r2/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
