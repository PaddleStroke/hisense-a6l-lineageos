...
class WriterChecks(unittest.TestCase):
    def test_geometry_and_extra_operations_rejected(self):
        ...
        for wrong in [protocol.PROGRAM_XML.replace('671744', '671745'),
    ...
    def test_cross_mode_payload_and_unrecognized_modes_rejected(self):
        ...
        directory = Path(__file__).resolve().parents[1] / 'firmware/extracted/recovery-v71-reinstall-20260921'
        if not directory.exists():
            directory = Path(__file__).resolve().parent
            files = [('install-diagnostic', directory / 'ram-staging/boot-magisk-staged-bootm1.img'),
                     ('restore-stock', directory / 'stock-boot/restore-stock-boot.img')]
        else:
            d2 = directory.parent / 'boot-magisk-20260921'
            files = [('install-diagnostic', d2 / 'boot-magisk.img'),
                     ('restore-stock', d2 / 'restore-stock-boot.img')]
