...
from DiagnosticRecoveryProtocol import check_program_xml, program_recovery, verify_payload, TARGETS
...
def main():
    ...
    capture_name = 'capture-diagnostic-install-v2' if args.mode == 'install-diagnostic' else 'capture-diagnostic-restore-v2'
    ...
    try:
        ...
        payload = ((home / 'ram-staging/recovery-diagnostic-unsigned.img').read_bytes()
        ...
        for name, (offset, size) in REGIONS.items():
            ...
            if name == 'recovery' and args.mode == 'install-diagnostic' and digest != RECOVERY_HASH:
                raise ValueError('Recovery differs from the verified stock restore image')
            print('Verified fixed read: ' + name, flush=True)
        if args.mode == 'install-diagnostic' and any((args.output / 'misc-bcb.bin').read_bytes()):
