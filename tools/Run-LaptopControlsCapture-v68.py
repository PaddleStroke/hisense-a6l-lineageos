...
CAPTURE = ROOT / 'capture-controls-user-v68'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V68 combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v68', 'suspend', 'idle']):
            raise RuntimeError('V68 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v68/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '2448b101eb780b1630cc6fd7181315da50211de2504b08cd6dad9e34d44a7520'):
