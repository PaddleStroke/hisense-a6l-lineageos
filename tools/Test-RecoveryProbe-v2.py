...
OUT = ROOT / 'firmware/extracted/recovery-probe-v2-20260915'
...
def main():
    ...
    try:
        ...
        assert sha(candidate) == '9c368dba3dc9a3e70167114ea160f224750dc7a9c8d39b4bc72191dd51344ac3'
        report['candidate_sha256'] = sha(candidate)
        header = module('Test-CapturedAblHeader').check(candidate[:4096])
        report['header'] = header
        assert header['status'] == '0x0' and header['image_size'] == 11993088
        ...
        assert sha(kernel) == 'bce9e6d3b330bb4a77cbd30b5bff71f1c8b5ba94f71417b8e0a495425079fb43'
        assert payload[:offset] == (OLD / 'Image.gz-dtb').read_bytes()[:offset]
        report['gzip_member_identical_to_previously_emulated_image'] = True
        ...
        vbmeta = (ROOT / 'captures/stock-control-restore-laptop-v1-20260915/edl/vbmeta-after-write.bin').read_bytes()
        assert sha(vbmeta) == 'e48632a03179e68b3c059905f613d321b3b37aa789ba72a02b322dffde4f7350'
        avb = module('Test-CapturedAblAvb').verify({'vbmeta': vbmeta, 'recovery': candidate})
        report['avb'] = avb
