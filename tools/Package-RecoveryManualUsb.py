"""Package the tested V15 kernel/RAM pair; preserve all device-tree wiring."""
import gzip
import hashlib
...
OLD = ROOT / 'firmware/extracted/recovery-probe-staged-usb-v14-20260916'
RAM = ROOT / 'firmware/extracted/staged-usb-ramdisk-v15-20260916'
TEST = ROOT / 'firmware/extracted/staged-usb-probe-v15-20260916'
...
def main():
    ...
    assert sha(kernel) == test['kernel_sha256'] == 'a499602699e44c79ff308d9d3f39e362c53efe548deb4b19946d6d2c72b957de'
    assert struct.unpack_from('<Q', kernel, 8)[0] == 0x80000
    assert b'A6L_USB_CONNECT_HELD' in kernel and b'A6L_USB_CONNECT_RELEASE_BEGIN' in kernel
    ...
    assert sha(original) == 'ca4c452b707524bac03d92626f27322ffd1327646a3eba3b8f735abf72c45971'
    ...
    assert 'a6l_manual_usb' not in old_cmdline
    cmdline = old_cmdline + ' a6l_manual_usb=1'
    ...
    for start, end in [(8, 12), (16, 20), (64, 576), (576, 608), (608, 1632), (1636, 1644)]:
    ...
    report = {'packaging_passed': True, 'ready_to_flash': False,
              ...
              'scope': 'A6L opt-in manual USB connect gate and revised RAM timeline; identical V14 DT, voltage constraints and load addresses',
