...
CAPTURE = ROOT / 'capture-controls-user-v74r'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed V74R combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v74r', 'suspend', 'idle']):
            raise RuntimeError('V74R requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v74r/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '417164b78c3bc7c43eba3039caab240194b5a69000b78737c50c3503f398bcf9'):
