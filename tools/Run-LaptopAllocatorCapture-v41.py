...
CAPTURE = ROOT / 'capture-allocator-user-v41'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Reuse V38 recovery for V41 RAM allocator integration; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v41', 'suspend', 'idle']):
            raise RuntimeError('V41 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v38/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '54b0d7b2502b71d8493e4669f2081e46d9ec4ba7a23c275373d735134977cbbb'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '1880s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v38.py'),
                                     str(CAPTURE / 'serial'), '--seconds', '1800'],
