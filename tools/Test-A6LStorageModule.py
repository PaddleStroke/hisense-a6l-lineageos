...
ARCHIVE = ROOT / 'firmware/extracted/storage-module-probe-v19-20260916'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-storage-module-probe-v19-20260916')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/storage-module-ramdisk-v19-20260916/ramdisk.cpio.gz'),
            '-append', 'earlycon=a6lfb keep_bootcon console=ttyAMA0,115200 loglevel=8 panic=0 initcall_debug pd_ignore_unused a6l_manual_usb=1 a6l_usb_trace=1']
    ...
    try:
        ...
        checks = dict(init_milestones_complete=all(pos >= 0 for pos in positions),
                      ...
                      staged_schedule=b'A6L_STAGED_STORAGE_V19 config_after=0 bind_after=4 connect_after=8 serial_after=12 module_after=20 heartbeat=2' in data,
...
def main():
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 22 seconds; no physical USB emulation',
              ...
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/storage-module-ramdisk-v19-20260916/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
