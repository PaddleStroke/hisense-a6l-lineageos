#!/usr/bin/env python3
"""Read fixed A6L regions, require an empty recovery request, then power off."""
import argparse
...
def allowed_xml(data):
    ...
    if node.tag in ('nop', 'configure', 'getstorageinfo'):
        ...
        if node.get('value') not in ('reset', 'off') or node.attrib != {'value': node.get('value')}:
            raise ValueError('Only the audited normal reset/poweroff operations are allowed')
    elif node.tag == 'read':
...
def main():
    ...
    if args.output.resolve().parent != home / 'capture-recovery-poweroff-v1' or args.output.name != 'edl':
        parser.error('Output must be the fixed recovery-poweroff capture directory')
    os.umask(0o022)
    args.output.mkdir(mode=0o700, exist_ok=False)
    account = pwd.getpwnam('pierrelouis')
    os.chown(args.output, account.pw_uid, account.pw_gid)
    report = {'scope': 'Fixed EDL reads and audited poweroff; all program/erase/patch commands rejected',
              'started_utc': datetime.now(timezone.utc).isoformat(), 'operations': [],
              'readback_verified': False, 'poweroff_acknowledged': False}
    ...
    signal.alarm(120)
    app = None
    wire_state = {'raw_read_pending': False, 'configured': False, 'ambiguous': False}
    try:
        ...
        save()
        if (args.output / 'misc-bcb.bin').read_bytes() != b'\0' * 4096:
            raise ValueError('A recovery request or other boot message is present; do not test entry from this state')
        report['empty_misc_verified'] = True
        last_response.clear()
        reply = fh.xmlsend('<?xml version="1.0" ?><data><power value="off"/></data>')
        report['poweroff_acknowledged'] = bool(reply.resp) and last_response.get('value') == 'ACK'
        report['poweroff_response'] = dict(last_response)
    except (Exception, SystemExit) as error:
        report['error'] = str(error)
        print('EDL check stopped: ' + str(error), flush=True)
    finally:
        ...
        if (report.get('error') and wire_state['configured']
                and not wire_state['raw_read_pending'] and not wire_state['ambiguous']
    ...
    return 0 if report['readback_verified'] and report['poweroff_acknowledged'] and not report.get('error') else 1
