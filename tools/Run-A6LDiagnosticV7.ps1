...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v7-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v7.py';Capture='Run-LaptopProbeCapture-v7.py';Restore='Run-LaptopDiagnosticRestore-v7.py'}[$Action]
