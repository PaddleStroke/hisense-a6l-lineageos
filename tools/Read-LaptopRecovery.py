#!/usr/bin/env python3
"""Read fixed A6L firmware regions through EDL; reject all persistent commands."""
import argparse
...
def main():
    ...
    if args.output.resolve().parent != home / 'capture-recovery-access-v4' or args.output.name != 'edl':
        parser.error('Output must be the fixed recovery-access capture directory')
    os.umask(0o022)
    args.output.mkdir(mode=0o700, exist_ok=False)
    account = pwd.getpwnam('pierrelouis')
    os.chown(args.output, account.pw_uid, account.pw_gid)
    report = {'scope': 'Fixed EDL reads and normal reset only; all program/erase/patch commands rejected',
    ...
    try:
        ...
        save()
        # All reads acknowledged and completed: normal reset is now permitted.
