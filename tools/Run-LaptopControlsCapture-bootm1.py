...
CAPTURE = ROOT / 'capture-controls-user-bootm1'
SERIAL = '1e529013'
...
def main():
    ...
    report = {'scope': 'Test installed BootM1 combined controls with unchanged V38 RAM userspace; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-bootm1', 'suspend', 'idle']):
            raise RuntimeError('BootM1 requires an active desktop idle/suspend inhibitor')
        install = json.loads((ROOT / 'capture-diagnostic-install-user-bootm1/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == '259c43e3f4c32ab95db02467d1bd34f5f0b525414f074583a715acc131f8237e'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '1880s',
                                     '/usr/bin/python3', str(ROOT / 'Wait-AndroidRamReady.py'),
                                     str(CAPTURE / 'readiness'), '--menu-seconds', '1800'],
                                    stdout=log, stderr=subprocess.STDOUT, timeout=1890)
