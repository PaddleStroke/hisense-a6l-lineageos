...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v9-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v9.py';Capture='Run-LaptopProbeCapture-v9.py';Restore='Run-LaptopDiagnosticRestore-v9.py'}[$Action]
