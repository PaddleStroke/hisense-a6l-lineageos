#!/usr/bin/env python3
"""Test writing the exact existing stock recovery, then independently read it back.

All other partitions remain read-only. This first validation mode requires the
pre-write recovery to match stock. It is not yet a restore-from-diagnostic mode.
"""
import argparse
...
from RecoveryWriteProtocol import check_program_xml, program_stock_recovery, verify_stock_payload

REGIONS = {
    'primary': (0, 1048576), 'tail': (125074122752, 22528),
    'recovery': (469762048, 67108864), 'devinfo': (253755392, 4096),
    'misc-bcb': (8757706752, 4096),
}
...
def allowed_xml(data):
    ...
    if node.tag in ('nop', 'configure', 'getstorageinfo'):
        ...
        if node.get('value') != 'reset':
            raise ValueError('Only a normal reset is allowed')
    elif node.tag == 'read':
...
def main():
    ...
    parser.add_argument('--stock-write-test', action='store_true', required=True)
    args = parser.parse_args()
    if os.geteuid() != 0 or socket.gethostname().split('.')[0] != 'system76-pc':
        parser.error('This bounded worker requires sudo on the established laptop')
    home = Path(__file__).resolve().parent
    kit = home / 'a6l-recovery-kit'
    if args.output.resolve().parent != home / 'capture-recovery-restore-v1' or args.output.name != 'edl':
        parser.error('Output must be the fixed recovery-restore capture directory')
    os.umask(0o022)
    args.output.mkdir(mode=0o700, exist_ok=False)
    account = pwd.getpwnam('pierrelouis')
    os.chown(args.output, account.pw_uid, account.pw_gid)
    report = {'scope': 'One write of the hash-pinned stock recovery, with before/after fixed reads and normal reset',
              'started_utc': datetime.now(timezone.utc).isoformat(), 'operations': [],
              'pre_write_readback_verified': False, 'write': {},
              'readback_verified': False, 'reset_acknowledged': False}
    ...
    wire_state = {'raw_read_pending': False, 'raw_write_pending': False, 'configured': False,
                  'ambiguous': False, 'permit_program': False, 'program_count': 0}
    try:
        # Hash the exact immutable payload before touching USB or loading code.
        payload = (home / 'stock-recovery/restore-stock-recovery.img').read_bytes()
        report['stock_payload_sha256'] = verify_stock_payload(payload)
        save()
        ...
        for name, (offset, size) in REGIONS.items():
            ...
            if name == 'recovery' and digest != RECOVERY_HASH:
                raise ValueError('Recovery differs from the verified stock restore image')
        ...
        save()
        # The known spare, both GPT regions and full existing stock recovery have
        # now been checked in this connection. Permit exactly one fixed program.
        wire_state['permit_program'] = True
        program_stock_recovery(fh, payload, report['write'], save)
        ...
        for name, (offset, size) in REGIONS.items():
            ...
            save()
            if digest != report['regions'][name]['sha256']:
                raise ValueError('Post-write bytes differ from verified pre-write region: ' + name)
        report['write']['independent_readback_verified'] = True
        report['readback_verified'] = True
        save()
        # Write and independent reads completed; normal reset is now permitted.
        report['reset_acknowledged'] = bool(fh.xmlsend('<?xml version="1.0" ?><data><power value="reset"/></data>').resp)
    except (Exception, SystemExit) as error:
        report['error'] = str(error)
        print('Stock recovery write test stopped: ' + str(error), flush=True)
    finally:
        # A rejected host-side command has not entered raw mode. When the
        # configured connection remains unambiguous, return to Android even if
        # verification stopped. Never reset in the middle of a raw transfer.
        if (report.get('error') and wire_state['configured']
                and not wire_state['raw_read_pending'] and not wire_state['raw_write_pending']
                and not wire_state['ambiguous'] and not report['write'].get('raw_mode_pending')
                and app is not None and app.fh is not None and app.cdc.connected):
            try:
                report['reset_after_stopped_check'] = bool(app.fh.firehose.xmlsend(
                    '<?xml version="1.0" ?><data><power value="reset"/></data>').resp)
            except BaseException as error:
                report['reset_after_stopped_check_error'] = str(error)
        signal.alarm(0)
    ...
    return 0 if report['readback_verified'] and report['reset_acknowledged'] and not report.get('error') else 1
