...
CAPTURE = ROOT / 'capture-probe-serial-user-v23'
SERIAL = '1e529013'
...
def main():
    ...
    try:
        ...
        install = json.loads((ROOT / 'capture-diagnostic-install-user-v23/edl/report.json').read_text())
        if not (install.get('mode') == 'install-diagnostic' and install.get('readback_verified')
                ...
                and not install.get('error') and install.get('target_sha256') == 'fcc665755df0dfb3324f606701f8072dcdda3cef98227e416c593ba9ff113afa'):
        ...
        with (CAPTURE / 'worker.log').open('x') as log:
            result = subprocess.run(['/usr/bin/timeout', '--signal=TERM', '--kill-after=3s', '340s',
                                     '/usr/bin/python3', str(ROOT / 'Collect-ProbeSerial-v23.py'),
