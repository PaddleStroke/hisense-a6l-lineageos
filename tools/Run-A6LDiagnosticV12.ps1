...
$resultFile = Join-Path $accessDir ('interactive-diagnostic-v12-' + $Action.ToLowerInvariant() + '-result.json')
$knownHosts = (Join-Path $taskRoot 'logs/a6l-laptop-known-hosts').Replace('\', '/')
$keyPath = (Join-Path $accessDir 'a6l_ed25519').Replace('\', '/')
$remoteName = @{Install='Run-LaptopDiagnosticInstall-v12.py';Capture='Run-LaptopProbeCapture-v12.py';Restore='Run-LaptopDiagnosticRestore-v12.py'}[$Action]
