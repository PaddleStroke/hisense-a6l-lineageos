...
V46='417164b78c3bc7c43eba3039caab240194b5a69000b78737c50c3503f398bcf9'  # here: the V68 tools' candidate
V45='b93771e701d677c58e377feae794a25d3a9c4c1f5cd379bb904910e4661a4373'  # here: the V68 tools' predecessor
report=json.loads((IMAGE/'report.json').read_text());V68=report['candidate_sha256']  # = V71 hash in this generator
...
pins=json.loads((T/'diagnostic-user-v71-tools.json').read_text())['files']
...
def v68(text):
    ...
    text=text.replace('verified-v70','@@LABEL@@').replace('verified V70 predecessor','@@PRED@@')
    text=text.replace('recovery-v71-candidate-20260921','recovery-rootedstock-20260921')
    return text.replace('V71','V72').replace('v71','v72').replace('@@LABEL@@','verified-v71').replace('@@PRED@@','verified V71 predecessor')
def put(name,text):
    p=T/name;p.write_text(text);py_compile.compile(str(p),doraise=True);manifest['files'][name]=digest(p)
for old in [n for n in pins if n!='diagnostic-user-v71-tools.json']:
    put(old.replace('V71','V72').replace('v71','v72'),v68(source(old)))
put('diagnostic-user-v72-tools.json',json.dumps(manifest,indent=2)+'\n')
