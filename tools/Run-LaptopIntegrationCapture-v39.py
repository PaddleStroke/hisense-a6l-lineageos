...
def main():
    ...
    report = {'scope': 'Reuse V38 recovery for V39 RAM integration; one bootloader restart, no image writes',
    ...
    try:
        ...
        if not all(word in inhibition for word in ['A6L-v39', 'suspend', 'idle']):
            raise RuntimeError('V39 requires an active desktop idle/suspend inhibitor')
