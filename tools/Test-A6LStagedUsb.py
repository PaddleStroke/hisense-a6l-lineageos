...
ARCHIVE = ROOT / 'firmware/extracted/staged-usb-probe-v13-20260916'
# Keep actively written QEMU logs on Linux ext4, not the Windows shared mount.
OUT = Path('/home/a6l/kernel/test-staged-usb-probe-v13-20260916')
...
'''
    ...
    args = ['qemu-system-aarch64', '-machine', 'virt,gic-version=3', '-cpu', 'cortex-a53',
            ...
            '-initrd', str(ROOT / 'firmware/extracted/staged-usb-ramdisk-v13-20260916/ramdisk.cpio.gz'),
            '-append', 'earlycon=a6lfb keep_bootcon console=ttyAMA0,115200 loglevel=8 panic=0 initcall_debug pd_ignore_unused']
    ...
    try:
        with (p / 'console.log').open('wb') as log, (p / 'stderr.log').open('wb') as err:
            ...
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                ...
                while time.monotonic() < until:
                    ...
                    if (any(int(v) >= 62 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+) ', data))) or b'Kernel panic' in data:
                        break
        ...
        checks = dict(init_milestones_complete=all(pos >= 0 for pos in positions),
                      ...
                      alive=any(int(v) >= 62 for v in re.findall(rb'A6L_RAM_PROBE_ALIVE seconds=([0-9]+) ', data)),
                      staged_schedule=b'A6L_STAGED_USB_V13 bind_after=20 serial_after=40 heartbeat=2' in data,
                      no_usb_without_controller=b'A6L_USB_CONFIG_BEGIN' not in data and b'A6L_SERIAL_OPEN_BEGIN' not in data,
...
def main():
    ...
    result = {'scope': 'Diskless QEMU test of staged RAM PID1 through 62 seconds; no physical USB emulation',
              'kernel_sha256': hashlib.sha256(adapted).hexdigest(),
              'ramdisk_sha256': hashlib.sha256((ROOT / 'firmware/extracted/staged-usb-ramdisk-v13-20260916/ramdisk.cpio.gz').read_bytes()).hexdigest(), 'cases': {}}
    try:
