"""Launch one verified V74C capture under the laptop's desktop sleep inhibitor."""
import json,os,subprocess
from pathlib import Path
root=Path(__file__).resolve().parent
assert not (root/'capture-controls-user-v74c').exists()
...
with (root/'controls-v74c-launch.log').open('x') as log:
    p=subprocess.Popen(['/usr/bin/gnome-session-inhibit','--app-id','A6L-v74c-capture',
                        '--reason','A6L combined controls test','--inhibit','suspend:idle',
                        '/usr/bin/python3',str(root/'Run-LaptopControlsCapture-v74c.py')],
